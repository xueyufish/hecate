...
#### Scenario: REST tool execution via spec-driven HTTP
- **WHEN** `registry.execute("crm__get_contact", {"contact_id": "42"}, context)` is called and the tool has `source="rest"` with a resolvable target
- **THEN** the registry SHALL route the call to the REST executor, which translates the arguments into an HTTP request against the target per the operation specification and returns the response payload

#### Scenario: REST tool with unresolvable target
- **WHEN** `registry.execute("crm__get_contact", args, context)` is called and the tool's target reference is missing or inactive
- **THEN** the registry SHALL raise a structured error indicating the gateway target is unavailable
