...
5. **模型调试场** — 内置模型测试面板，选择模型后输入 prompt 测试响应质量，支持调参数（temperature、max_tokens）
6. **Provider 级配置** — 每个 Provider 可配置 timeout、retry 次数、rate limit
7. **模型降级集成** — Provider 状态变化时自动更新关联 Agent 的 fallback 链
...
├── api_key (string, encrypted via Fernet)
├── base_url (string, optional) — 自定义端点
├── config (JSON) — {"timeout": 30, "max_retries": 3, "rate_limit_rpm": 60}
├── is_enabled (bool)
...
# 模型调试（管理员）
POST   /api/models/test              — 选择模型 + 输入 prompt + 参数，测试响应
...
3. **设置 → 模型调试** — 模型测试面板，选模型、输 prompt、调参数、看响应
4. **创建 Agent 模型选择** — 改造为按 Provider 分组的下拉组件
...
- API Key 使用 Fernet 对称加密存储，密钥从 `FERNET_KEY` 环境变量读取（未设置时明文存储兼容开发环境）
- Provider 状态变化时，检查关联 Agent 是否需要更新 fallback 链
