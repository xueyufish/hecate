# skill-dependency-declaration Specification
...
`requires` SHALL 默认拒绝跨 source 引用。允许矩阵:
...
agent 版本「提交版本」流程 SHALL 在 pin 直接列出的 skill 后,递归解析每个 skill 的 `requires`,把所有解析出的 skill(直接 + 隐式)按 provider precedence(`project > user > bundled`)解析到具体 `(name, skill_id, provider, version, content_hash)` 五元组,并写入 agent 版本 ref-manifest 的 skill 条目。
...
绑定期闭包 walk 若任何依赖项无法解析(workspace 中不存在 / 跨 source 不兼容 / 当前 provider 下活行被软删),agent 版本提交 SHALL hard-fail,错误信息点名全部缺漏节点与各自的传递路径。

整个写入操作 SHALL 在数据库事务内完成,任一节点解析失败即回滚,**绝**不部分写入 ref-manifest,**绝**不静默跳过任何依赖项。
...
Agent Plugins 1.0 `plugin.json` SHALL 通过 Hecate namespace 扩展(`extensions.io.github.xueyufish`)支持 `requires` 字段,plugin-level require 在该 plugin 包内所有 skill 间共享,适用与 skill 级 `requires` 相同的图校验规则(缺失依赖、环、跨 source)。
...
- **THEN** 整个 plugin 安装被拒,错误信息包含 missing-helper,不写入 PluginModel
