...
## Purpose
Hecate CLI (`hecate`) 是平台的用户面控制台入口,通过 `typer` 子命令组暴露 skills / agents / sessions / workflows / prompts / memory / kb / tools 等资源的 CRUD 与诊断能力,使用 `httpx` 与后端服务通信并支持 `--json` / `--page` 等机器可读输出。
## Requirements
