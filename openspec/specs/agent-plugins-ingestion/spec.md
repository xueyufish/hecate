...
Each installed package SHALL persist exactly one PluginModel row with `type="agent-plugin"`, status, workspace scope, provenance fields `origin` (source descriptor including git pin triple when applicable), `content_hash`, and `scan_result` (reserved, null until feature 5.13a), and a manifest JSON containing the full plugin.json plus a component inventory listing each skill and MCP server with its import outcome. The install directory and the database row SHALL be created and removed as a pair; at startup, install directories without a matching row SHALL be removed as orphans.
...
The ingestion pipeline SHALL invoke a scan stage between validation and persistence, and re-invoke it on enable. Until feature 5.13a ships, the stage SHALL be a no-op that leaves `scan_result` null. The stage interface SHALL accept package content and return findings without blocking installs in this phase.

#### Scenario: No-op scan passes through
- **WHEN** a package installs while the scan stage is a no-op
- **THEN** installation proceeds and `scan_result` remains null

### Requirement: Master switch
All ingestion entry points (REST API and CLI) SHALL be gated by a configuration switch defaulting to off. When off, install endpoints SHALL return a feature-disabled error and the CLI SHALL report the feature disabled. The switch SHALL also serve as an emergency kill-switch after go-live.

#### Scenario: Default-off rejects install
...
#### Scenario: Switch enabled allows install
- **WHEN** the switch is enabled and a valid package is installed
