...
The system SHALL define `ToolRule` as a dataclass with three fields: `action` (RuleAction enum: ALLOW/DENY/ASK), `pattern` (str, tool-name glob), and `priority` (int, default 0).

#### Scenario: Rule construction
- **WHEN** `ToolRule(action=RuleAction.DENY, pattern="terminal(rm *)")` is constructed
- **THEN** `action` is `RuleAction.DENY` and `pattern` is `"terminal(rm *)"`
...
The system SHALL define `ToolAccessPolicy` as a concrete class in `engine/tool_access.py` with an `evaluate(tool_meta: dict, rules: list[ToolRule], context: dict) -> AccessDecision` method that applies three-layer evaluation.
...
The `evaluate` method SHALL check rules in order: DENY first, then ASK, then ALLOW, then risk-level fallback. The first matching rule wins.
...
- **WHEN** rules contain both `ToolRule(ASK, "write_file(.env*)")` and `ToolRule(ALLOW, "write_file(*)")`
- **AND** tool name is `"write_file(.env.production)"`
- **THEN** the result is `AccessDecision.REQUIRE_APPROVAL`
