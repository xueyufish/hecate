...
#### Scenario: DYNAMIC value is lowercase string
- **WHEN** `CollaborationPattern.DYNAMIC.value` is accessed
- **THEN** it SHALL return `"dynamic"`

### Requirement: Pattern inference from graph structure
The system SHALL provide an `infer_pattern(config: GraphConfig) -> CollaborationPattern | None` function that analyzes graph topology to detect the collaboration pattern. The function returns `DYNAMIC` when the graph contains a node of type `COORDINATOR`; it returns `None` if no known pattern matches.

#### Scenario: Dynamic pattern detected via COORDINATOR node
- **WHEN** `infer_pattern()` receives a GraphConfig that contains at least one node with `type == "coordinator"` regardless of other node types
- **THEN** it SHALL return `CollaborationPattern.DYNAMIC`

#### Scenario: Sequential pattern detected
- **WHEN** `infer_pattern()` receives a GraphConfig with a linear chain of 3+ AGENT nodes connected by unconditional edges, no FAN_OUT/MERGE nodes, no COORDINATOR node, and no handoff edges
- **THEN** it SHALL return `CollaborationPattern.SEQUENTIAL`

#### Scenario: Parallel pattern detected
- **WHEN** `infer_pattern()` receives a GraphConfig containing at least one FAN_OUT node and one MERGE node and no COORDINATOR node
- **THEN** it SHALL return `CollaborationPattern.PARALLEL`

#### Scenario: Handoff pattern detected
- **WHEN** `infer_pattern()` receives a GraphConfig where all edges have `trigger="handoff"` and no COORDINATOR node is present
- **THEN** it SHALL return `CollaborationPattern.HANDOFF`

#### Scenario: Broadcast pattern detected
- **WHEN** `infer_pattern()` receives a GraphConfig where all agent nodes share read/write access to a single TOPIC channel, edges form a sequential chain without FAN_OUT/MERGE, and no COORDINATOR node is present
...
- **WHEN** `infer_pattern()` receives a GraphConfig with 2+ agent nodes, a round counter ACCUMULATOR channel, alternating execution edges forming a loop, and no COORDINATOR node
- **THEN** it SHALL return `CollaborationPattern.DEBATE`

#### Scenario: Unknown pattern returns None
- **WHEN** `infer_pattern()` receives a GraphConfig that does not match any known pattern and contains no COORDINATOR node
...
#### Scenario: Dynamic pattern not buildable statically
- **WHEN** `build_graph_from_pattern(CollaborationPattern.DYNAMIC, {"goal": "..."})` is called
- **THEN** the function SHALL raise `NotImplementedError` because dynamic graphs are runtime-emitted by a COORDINATOR node and cannot be statically constructed

### Requirement: Pattern metadata endpoint
The system SHALL expose `GET /api/collaboration-patterns` returning a list of all 7 pattern definitions with name, description, required parameters, and preview metadata.

#### Scenario: List all patterns
- **WHEN** `GET /api/collaboration-patterns` is called with a valid API key
- **THEN** the response SHALL contain an `items` array with exactly 7 entries, each having `id`, `name`, `description`, `parameters` (JSON Schema for configuration), and `preview` (node count, edge count estimates)
