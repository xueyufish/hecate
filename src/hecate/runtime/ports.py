...
This zero-dependency design keeps the runtime framework-agnostic and allows
...
class SpanContext:
    ...
    Carries the IDs needed to correlate runtime events with application-level traces.
    """
...
class RuntimePort(ABC):
    ...
    async def create_span(
        ...
        Called by the runtime at execution boundaries (node start/end, LLM calls,
    ...
    async def llm_invoke_structured(
        ...
        Enables the runtime layer's Pregel chat graph tool-calling loop to detect when
