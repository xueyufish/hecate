...
structured tool policy decision events from the runtime layer without depending
on ``models/`` or ``services/``.

Design (see openspec/changes/siem-security-pipeline/design.md D1):
- ``DecisionSink`` is the runtime-layer interface (like ``RuntimePort``).
...
# Module-level singleton — runtime code imports this directly.
