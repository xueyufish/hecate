...
| # | Feature | Dependencies | Effort |
|---|---------|------|--------|
| 4.5 ✅ (2026-09-20, `memory-consolidation`, archive `2026-09-21-memory-consolidation`) | Sleep-time Memory Consolidation — plan-then-apply engine (extract → typed ops → code-applied via shared write path), SUPERSEDE lineage + consolidation_runs audit/watermark, trigger bus (cron 02:00 default + idle sweep + pressure-flag priority, advisory-lock multi-instance), candidate security scan, in-app cosine (exact-text degradation), per-agent L1 allowlist (default `learned_context` only); behind `CONSOLIDATION_ENABLED` | Memory System ✅ | M |
...
| 4.16 ◐ | LLM-Managed Memory — tool layer ✅ (2026-09-19, `agent-memory-tools`): 8 memory tools + tiered MemoryProvider contract + prefetch processor; remaining: lifecycle UX | ContextEngine ✅ | M→S |
...
LLM-Managed Memory (4.16 ◐ tool layer ✅) → Memory Pressure Alert (4.17 ✅) → ContextEngine Integration (✅ via 4.13 chain processors)
