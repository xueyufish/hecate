...
| 5.9c ✅ (2026-09-23) | Skill Auto-Detection — workspace 内符合条件的 skill 无需手动 agent-skill 绑定即可被模型发现:L1 catalog 注入发现池(workspace + bundled、`model_invocable`、非 `user` provider、plugin 启用、precedence 归并、trust 下限过滤),`load_skill` advertised 集合放宽为 绑定 ∪ 发现池;触发 = LLM 读 description 自选(渐进披露既有),激活维持 run-scoped。三层治理只收窄:全局 `SKILL_DISCOVERY_ENABLED`(默认关,升级零变化)→ workspace `settings.skill_discovery`(enabled + `min_trust_tier`,默认 community)→ per-agent `skill_discovery_enabled`(None=跟随 / False=退出)。catalog 预算 1000→2000 tokens,溢出选择确定性(绑定/auto_load 恒优先 → trust tier → provider rank → `skill_loaded` 使用次数 → 名称)。发现 = 活面(活行解析,不进 ref_manifest 与漂移报告),绑定 = 冻结面;`POST /api/agents/{id}/skills/promote` 转绑定并提示 commit 版本冻结。usage 事件带 `detected_via`(bound/auto_detected);发现的 skill `requires` 不可解析时照常服务并记 `dependency_warning` 事件(5.9e 绑定期契约不变)。**Deferred**:`search_skills` 长尾词法检索工具(→5.9f)、description 触发质量 lint/评估线束、per-skill 上下文成本与使用率观测、suggest-binding 闭环、`user` provider 成员级隔离(安全线)。**Shipped 2026-09-23**(全局默认关,灰度经 workspace `settings.skill_discovery.enabled` 放量;studio UI 与 CLI 延后;archive `openspec/changes/archive/2026-09-24-skill-auto-detection/`) | 5.9-enh ✅ + 5.9e ✅ + 1.3.20 ✅ | M |
| 5.9f | Skill 长尾词法检索 — `search_skills(query)` builtin:模型自调的确定性词法检索(词项覆盖排序、有界返回),承接 catalog 预算容纳不下的超大规模 workspace skill 池;零 embedding 依赖 | 5.9c ✅ | S |
...
| 4.14 ✅ (2026-09-21) | Memory Importance Scoring → P4 (delivered via `memory-importance-fusion`, archive `2026-09-21-memory-importance-fusion`: consolidation planning prompt anchor-tier scoring with `[0,1]` clamp + neutral 0.5 default; write-time only) | Memory System ✅ | M |
| 4.15 ✅ (2026-09-21) | Multi-Signal Fusion Retrieval → P4 (delivered via `memory-importance-fusion`, archive `2026-09-21-memory-importance-fusion`: relevance-dominated ranking with bounded multiplicative bias `relevance × decay × importance` clamped to 0.3× floor; `MEMORY_FUSION_BIAS_ENABLED` default off; per-signal `breakdown` exposed in `MemoryFactHit.metadata`; per-session prefetch pinning for KV-cache stability; access frequency excluded from query-time formula) | 4.14 ✅ | M |
| 4.17 ✅ (2026-09-20) | Memory Pressure Alert → P4 (delivered via `memory-consolidation`, archive `2026-09-21-memory-consolidation`: latched `[memory_pressure]` nudge at 0.9 + consolidation pressure mark) | Token Budget ✅ | S |
| 4.25 ✅ (2026-09-24) | Layered Memory System → P4 — flush + lifecycle (rescoped) | 1.3.15 ✅ | M |
...
| 2.6a + 1.1.21 + 1.3.10⊕6.23 (+6.49) ✅ (2026-09-13) | Multi-Agent Controller Family — central controller (2.6a), controller canvas (1.1.21), 5-Level Intent Recognition (6.23 merges 1.3.10), Intent Package Asset (6.49 — intent categories + sample utterances as few-shot evidence for the recognition engine) | 2.6 (same Sprint) + 2.7c intent routing ✅ | M×3 + M | AgentArts 控制器 = 子智能体/子工作流组合调度; 一族三编号跨 Sprint 8/9 收口为一个 change; 6.49 为 6.23 的配套数据面; 底座 ✅; 已交付 (archive `2026-09-13-intent-recognition`; L5 元意图按调研裁决以确定性策略门交付, 自动调参闭环为 follow-up) |
| 1.3.20 ✅ (2026-09-14) | Agent Versioning & Channel Publishing — agent-level versioning + channel binding (API/im 锁定版本快照; embed/webhook v1 仅占位枚举); carries Resource Versioning (14.x) mechanism referenced by 5.9d / 3.5.12 / 7.5 | 1.1.9 ✅ + channels ✅ + 1.3.15 ✅ | M | 唯一规划外空白; AgentArts 把版本+渠道做成三种形态统一生命周期步骤 (archive `2026-09-14-agent-versioning`) |
...
| # | Feature | Dependencies | Effort |
|---|---------|------|--------|
| 1.3.5e ◐ (Stage 2 ✅ 2026-09-18, archive `2026-09-18-grounding-scoring-layer`) | Hallucination Detection & Mitigation — Stage 1 citation provenance layer（`【N-M】` 带内标记 + session 级注册表 + 响应回映射 + 引用率 warn 信号 + `CITATION_*` 审计事件）＋ Stage 2 grounding scoring layer（`GroundingScorer` 双后端 seam：LLM judge 走模型调用路径 + HTTP NLI 端点（MiniCheck 类自托管，契约见 `docs/operations/grounding-nli-endpoint-contract.md`）、D8 事实句 claim 清单、`knowledge_query` 兜底检索（独立 `fallback_kb_ids`）、三态裁决（低置信 ≠ contradicted）、`GROUNDING_SCORE` 审计事件含 `would_block` 影子判定、judge 升级路径、per-node `grounding_scoring` 策略默认关闭、LLMWorker 补齐 `LLM_RESPONSE` 链执行（legacy hook 经 adapter 等价入链）、studio 引用徽标）；Stage 3（处置动作 pass/warn/block/regenerate）另行立 change，立项前置 = 校准 runbook（`docs/operations/grounding-score-calibration.md`）产出的冤枉率/漏报率数据 | PostLLMHook ✅ + ContextEngine ✅ (4.13) + Stage 1 ✅ | L |
...
| # | Feature | Dependencies | Effort |
|---|---------|------|--------|
| 4.5 ✅ (2026-09-20, `memory-consolidation`, archive `2026-09-21-memory-consolidation`) | Sleep-time Memory Consolidation — plan-then-apply engine (extract → typed ops → code-applied via shared write path), SUPERSEDE lineage + consolidation_runs audit/watermark, trigger bus (cron 02:00 default + idle sweep + pressure-flag priority + reflection runs + 4.25 `pre_compaction` flush windows; advisory-lock multi-instance; also hosts the 4.25 lifecycle sweeper), candidate security scan, in-app cosine (exact-text degradation), per-agent L1 allowlist (default `learned_context` only); behind `CONSOLIDATION_ENABLED` | Memory System ✅ | M |
| 4.13 ✅ (2026-09-16; durable compaction ✅ 2026-09-19) | Context Engine Processor Chain — 可插拔上下文处理器链（``ContextProcessorChain`` + ``context_policy`` 策略解析）：原子 tool-call 分组、TokenEstimator（provider anchor > legacy engine > 启发式）、预算告警 HintBlock、KV-cache 感知前缀保护 + ``cache_hint`` 经 4.11 shaping 渲染、窗口选择（安全区内 importance 排序）、offload + ``recall`` 工具、投影/表面替换压缩双后端接缝、执行器级 FailurePolicy（熔断/冷却/anti-thrash）、受控终止 ``token_capped`` 取代静默截断、策略解析 node > agent > model-capability > platform（fail-fast 校验 + canonical_hash 入 agent 版本化）、BUDGET_SNAPSHOT 词汇表增量（``warn``/``terminate``）。durable compaction（surface replacement 后端）✅ (2026-09-19) 已按 ADR-033 落地（archive `2026-09-19-durable-compaction-surface-replacement`）：bracket 事件、影子化账本视图（端点锚点 fail-open + 触发自愈）、容量轴触发（有效 surface × ``trigger_ratio``，独立于预算轴 satisfaction 短路）、bracket 锁（TURN_END stale / COMPACTION_FAILED 消解 + per-session 进程内锁）、滚动再压缩（``prior_compaction_id`` 链）、eviction 互斥（装配期 fail-fast）；``CompactionSummarizer`` seam（路由 pin 与 cache namespace 隔离留在 composition）（archive `2026-09-16-context-processor-chain`） | ContextEngine ✅ + 1.3.19 | M |
| 4.16 ✅ (2026-09-24) | LLM-Managed Memory — tool layer ✅ (2026-09-19, `agent-memory-tools`); lifecycle UX ✅ (2026-09-24, `memory-lifecycle-governance`): governance REST + Memory Center + per-workspace/agent memory policies | ContextEngine ✅ | M→S |
...
| # | Feature | Dependencies | Effort |
|---|---------|------|--------|
| 1.1.22 | Orchestration Mode Switching | 1.1.21 ✅ (delivered in `intent-recognition`) | M |
...
Memory Isolation (4.6) → Memory Importance Scoring (4.14 ✅ 2026-09-21) → Multi-Signal Fusion Retrieval (4.15 ✅ 2026-09-21)
LLM-Managed Memory (4.16 ✅) → Memory Pressure Alert (4.17 ✅) → ContextEngine Integration (✅ via 4.13 chain processors)
...
Skill Auto-Detection (5.9c ✅) → Skill 长尾词法检索 (5.9f) → Context-Based Skill Invocation
...
Memory Integration → Sleep-time Consolidation (KM3 ✅ via 4.5, 2026-09-20) → Overnight Synthesis
