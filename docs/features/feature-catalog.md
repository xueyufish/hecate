...
| 1.3.19 | Event-Sourced Execution State / Log-as-Truth (NEW — 2026-08-14) | Agent Runtime | Upgrade EventStore from observation log to state carrier: "model-visible ⟺ logged" runtime invariant — anything reaching a model request must be reconstructable from the event log; model context projected from the log (derive_messages); checkpoint = event-replay fold, snapshots demoted to materialized caches; DeltaChannel incremental storage cuts long-run storage from O(N²) to near-linear. Foundation for 8.20 Run Replay, HITL durable audit pairs, and middleware waterfall events. **Depends on**: 8.8 EventStore ✅, CheckpointStore ✅. | dsh session log (source-verified v0.1.0-rc.5), deer-flow DeltaChannel (v2.0.0), MAF Durable extension, OMA checkpoint+resume |
