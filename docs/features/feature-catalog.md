...
| 5.5d | Dual-Format Plugin Convergence & Export (NEW — 2026-08-16) | Tools & Plugins | Single-package dual-format via the Agent Plugins `extensions` escape hatch: Hecate-private content (type/entry/permissions/config_schema + source) migrates into an `io.hecate/` reverse-domain namespace directory inside an Agent Plugins package (spec assigns no semantics to namespace contents; other clients ignore them) — one package is simultaneously a conformant Agent Plugins plugin (skills/mcp.json visible to all clients) and a Hecate deep-integration plugin. Eliminates dual-format friction without inventing a third format. Also `hecate plugin export`: package workspace skills as a conformant Agent Plugins bundle for use in Cursor/Codex etc. (bidirectional ecosystem play). Includes dropping forced-ZIP install in favor of directory/git URL (ZIP as transport only). Namespace domain must be one Hecate controls — confirm before implementation (decided 2026-08-16; io.hecate is a placeholder). **Depends on**: 5.5c (P3) + 5.5b ✅. Non-blocking enhancement. | Agent Plugins 1.0 spec §Client Extensions, npm/OCI "interchange envelope" pattern, Claude Code marketplace (git-repo format) |
