...
| 1.3.2 | Tool Calling ✅ | Agent Runtime | Agent calls tools with parallel execution, retry, and timeout support. When the assistant proposes multiple tool calls in one turn, ``ToolWorker.execute`` and the chat path-A loop dispatch them concurrently via ``asyncio.gather``; result ordering matches the LLM's submission order regardless of completion order. | Claude Code (40+ tools), OpenClaw (TypeBox) |
...
| 1.3.4 | Human Intervention ✅ | Agent Runtime | Execution pause, human approval, result correction, resume continue. **Result correction wiring (delivered)**: the Pregel runtime writes the human-supplied resume value into the ``_resume_value`` channel on restore (``_restore_from_checkpoint``); ``LLMWorker.execute`` and ``execute_stream`` consume it via ``_consume_resume_value`` and materialize it as user-role messages appended to the… archive `openspec/changes/archive/2026-08-21-guardrail-upgrade-trio` | Bisheng (workflow pause), LangGraph (interrupt), dsh approval seam (source-verified), MAF durable approvals, AgentScope PermissionEngine |
| 1.3.5 | Error Recovery ✅ | Agent Runtime | Tool failure retry, model degradation, graceful degradation. | OpenClaw (14 failure types) |
| 3.1.1 | Document Parsing ✅ | Knowledge Base/RAG | PDF/Word/PPT/Excel/HTML/Markdown parsing. **Office Open XML supplement (P1 audit, this branch)**: `.pptx` text extraction via stdlib zip+XML over DrawingML `<a:t>` runs (slides ordered numerically); `.xlsx` via shared-strings + worksheet XML resolution (shared `t="s"`, inline `t="inlineStr"`); corrupt archives raise `ValueError`; legacy binary `.ppt`/`.xls` remain unsupported (convert to OOXML… | Docling (20+ formats), RAGFlow DeepDoc |
| 3.2.1 | Vector Search ✅ | Knowledge Base/RAG | Embedding + ANN search. | LlamaIndex, Qdrant |
| 3.2.6 | Chunking Strategy ✅ | Knowledge Base/RAG | Auto-chunking, character-based, separator-based, semantic chunking. **Strategy supplement (P1 audit, this branch)**: `ChunkingStrategy` enum on `TextChunker` — `character` (default, legacy behavior preserved), `separator` (blank-line/structural split, fragments merged up to chunk_size, oversize falls back to character), `auto` (per-document detection: blank-line paragraphs or markdown headers →… | Coze (4 strategies), LlamaIndex |
...
| 1.1.4 | Workflow Test Run ✅ | Agent Development | Step debugging, input/output preview, execution logs. **Planned enhancement (G7)**: Agent Debug Inspector — superstep-level state inspector showing Channel values, node inputs/outputs, and execution timeline at each BSP barrier. | Dify, Coze, LangGraph Studio |
...
| 9.1a | Output Injection Type Detection ✅ | Security & Compliance | Detect downstream-system injection patterns in LLM output (9.1a): code (Python `exec`/`eval`/`__import__`/`compile`), SQL (`DROP TABLE`/`UNION SELECT`/`;DELETE`), template (Jinja SSTI `{{ config }}`/`{% import %}`), XSS (`<script>`/`onerror=`/`javascript:`). | Bedrock Guardrails Standard tier, DeerFlow SkillScan, OWASP LLM01:2025 Scenario #5 |
| 9.2 | System Prompt Leakage Protection ✅ | Security & Compliance | Detect and block system-prompt / instruction exfiltration attempts in LLM output (OWASP LLM07) — output-security leg complementing 9.1a's output-side SS3 detection (both inspect LLM responses before they flow to downstream systems or surface to users). | Lakera Red, Promptfoo, Amazon Bedrock Guardrails, OWASP LLM07:2025 |

### Observability Foundation

| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 8.1 | Full-Chain Tracing ✅ | Observability & Operations | Trace → Span → Generation hierarchical tracing with OTel context propagation, async SQLAlchemy persistence, and REST API. **Planned enhancement (OE2)**: OTel GenAI Semantic Conventions Alignment — standardize Span attributes to OpenTelemetry GenAI semantic conventions (`gen_ai.operation.name`, `gen_ai.request.model`, `gen_ai.agent.id`, `gen_ai.usage.total_tokens`, `gen_ai.tool.name`, etc.). | LangFuse (Session > Trace > Observation), OpenTelemetry GenAI Semantic Conventions (2025-2026) |
| 8.7 | Audit Logs ✅ (SIEM Pipeline ✅) | Observability & Operations | User operation audit trail, compliance requirements. **SIEM Pipeline (SS5) implemented**: structured security event export to enterprise SIEM systems via Webhook (Splunk HEC/Datadog/Elastic), Syslog (RFC 5424 TCP/UDP/TLS), and OCSF v1.5 formatter. | Cisco AI Defense (SIEM integration), Enterprise security standard (CEF/LEEF/OCSF) |
...
| 9.2 | System Prompt Leakage Protection ✅ | Security & Compliance | Detect and block system-prompt / instruction exfiltration attempts in LLM output (OWASP LLM07) — output-security leg complementing 9.1a's output-side SS3 detection (both inspect LLM responses before they flow to downstream systems or surface to users). | Lakera Red, Promptfoo, OpenClaw |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 2.8 | Collaborative Conflict Handling ✅ | Multi-Agent Orchestration | Conflict detection, locking, and resolution when multiple Agents access shared resources. 4 strategies: LAST_WRITE_WINS, HUMAN_APPROVAL, DISTRIBUTED_LOCK, NEGOTIATION. | OpenClaw (session lanes) |
| 2.9 | Unified Skill Registry ✅ | Multi-Agent Orchestration | Unify Tools, Knowledge Bases, Workflows, and sub-Agents as attachable "Skills". SkillRef/ResolvedSkill abstraction, SkillRegistry service with resolve/invoke/format_for_llm. | Copilot Studio (Tools + Topics + Knowledge), Agentforce (Actions + subagents), watsonx (Agent + Agentic Workflow), Coze (Bot + Workflow/Chatflow), ADK (LlmAgent + WorkflowAgent) |
| 2.9a | Agent-Workflow Mutual Embedding ✅ | Multi-Agent Orchestration | Agent can invoke Workflow as a Tool (Coze: Bot mounts Workflow as skill). Workflow can embed Agent as a DAG node with shared context (watsonx: Agent Node in Agentic Workflow). | Coze (Bot + Workflow), watsonx (Agent + Workflow DAG), Agentforce (Subagent + Actions), ADK (LlmAgent as sub_agent of WorkflowAgent), IBM watsonx (nesting depth warning) |
| 2.10 | A2A Protocol ✅ | Multi-Agent Orchestration | Google Agent-to-Agent protocol (Linux Foundation v1.0): standardized HTTP+JSON/gRPC messaging between agents across frameworks. AgentCard discovery (`/.well-known/agent-card.json`), task lifecycle (submitted→working→completed/cancelled), artifact exchange, push notifications. | Google A2A, ADK (A2A client/server), Salesforce (cross-org A2A), IBM (A2A integration) |
| 2.10a | Signed Agent Cards ✅ | Multi-Agent Orchestration | Cryptographic signatures on Agent Cards for identity verification (A2A v1.0). ES256 key pair generation (ECDSA P-256), JWS signing with RFC 8785 JSON Canonicalization Scheme, JWKS served at `/.well-known/jwks.json`, key rotation with grace period, signature verification with algorithm pinning (ES256 only), JWKS caching with configurable TTL. Status split: **server-side signing delivered** (JWKS endpoint + `A2A_SIGNING_*`); **client-side verification delivered** (fail-closed discovery against the `A2A_TRUSTED_JWKS(_PATH)` trust root, `AgentCard.verified` marker distinguishing verified from unverified fetches). | OpenAI Agents SDK (handoff_description), Google ADK (agent.description), AutoGen Swarm, LangGraph (Command(goto=...)) |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 11.2 | Web Widget (Simplified) ✅ | Multi-Channel Access | Full version of Web Widget for anonymous公开场景。**Status (2026-08-12, P5 deferred)**: WidgetModel + 临时 JWT 签发 + Origin 白名单 + JWT RS256 + JS bundle（参考 Intercom bundle 优化、Salesforce RS256、IBM watsonx 双 RSA 密钥、Google CX Agent Studio token broker 模式、Dialogflow Messenger Web Component 形态）。**Trigger = 第一个 to-C 公开网站场景（自部署用户做营销 / 客服 / Lead 收集）的明确客户需求**——11.2 简化版（P3 Wave 1）只覆盖内部 Portal /… | Coze, Dify, Intercom (JWT + bundle optimization), Salesforce Enhanced Web Chat (RS256 + identityToken API), Google Dialogflow Messenger (Web Component) |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 13.10 | Channel Update Conflict Resolution ✅ | Deployment & Operations | Conflict resolution for concurrent channel updates with 4 strategies: LAST_WRITE_WINS, MERGE_LIST, MERGE_MAP, HUMAN_APPROVAL — the ✅ covers the in-process ConflictResolver only; the Temporal durable-execution backend (`runtime/temporal/`) is a placeholder returning `status: "placeholder"`. | Temporal, CRDT |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 5.5a | Plugin SPI Core ✅ | Platform SPI | Plugin registration, discovery, lifecycle management, and sandbox isolation ABC. Defines PluginRegistry, PluginManifest, PluginLifecycle. | OpenClaw (100+ extensions), HermesAgent (plugin hooks) |
| 7.2-abc | EvaluatorBase ✅ | Platform SPI | Evaluator uniform interface: `evaluate(input, context) → EvalResult`. Existing 40+ evaluators (7.2a) become `BuiltinEvaluator` implementations; third-party evaluators register via Plugin SPI. | Ragas (evaluator abstraction), Google ADK (evaluation metrics) |
| 11.1-abc | ChannelBase ✅ | Platform SPI | Channel adapter uniform interface: `receive`, `respond`, `stream`. REST/WS/CLI (11.1, 11.7) become `BuiltinChannel` implementations; Feishu/DingTalk/Slack etc. | OpenClaw (channel plugin architecture), Coze (channel SDK) |
| 10.3-abc | AuthProvider ✅ | Platform SPI | Auth provider uniform interface: `authenticate(token, db) → AuthContext None`. JWT/APIKey (10.6) become `BuiltinAuthProvider` implementations; LDAP/OIDC/SAML register as plugins. | Enterprise standard (SAML/OIDC), Bisheng (SSO + LDAP) |
...
| 9.10 | Outbound DLP Engine ✅ | Security & Compliance | Outbound Data Loss Prevention for AI agent workflows — scans data before it leaves to external LLM providers, MCP servers, or third-party APIs. Three scan points: Pre-LLM, Post-Tool, Pre-Memory. | Control Zero (Gateway DLP), Pipelock (62 patterns), ORION Security (agentic DLP) |
...
| 10.8 | Enterprise Vault Integration ✅ | Multi-Tenant & Enterprise | Integration with enterprise secret management platforms: HashiCorp Vault, AWS Secrets Manager, Azure Key Vault, Google Secret Manager. Per-agent identity authentication to vault via OAuth 2.0 token exchange. | HashiCorp Vault, AWS Secrets Manager, Azure Key Vault |
...
|---|---------|--------|-------------|--------|
| 7.2b | AI-Synthesized Evaluation Dataset ✅ | Evaluation & Testing | LLM auto-synthesizes evaluation datasets from seed data, adversarial + security-compliance samples. **Shipped 2026-09-10** (archive `openspec/changes/archive/2026-09-10-ai-synthesized-evaluation-dataset/`): 16 built-in evaluators (scope four-layer taxonomy) + AI dataset synthesis pipeline (generation / evolution / adversarial × 5 OWASP-anchored intents) + sync/async job lifecycle + filter… **Shipped 2026-09-10** archive `openspec/changes/archive/2026-09-10-ai-synthesized-evaluation-dataset` | M |
| 7.2c | Online/Offline Evaluation Tasks ✅ | Evaluation & Testing | Persistent evaluation tasks in two kinds. **Offline task** = `EvaluationTask` definition (dataset × evaluators × `answer_source` manual/pipeline/agent) + async run lifecycle (202 + `pending→running→completed/failed`), optionally invoking the agent under test via `RuntimePort.agent_execute` per item, with threshold pass/fail + baseline regression `summary` on `evaluation_runs`. **Shipped 2026-09-11** archive `openspec/changes/archive/2026-09-11-online-offline-evaluation-tasks` | M |
| 7.2d | Trace Backflow Dataset ✅ | Evaluation & Testing | Automated backflow of scored production traces into evaluation datasets — the automated half of backflow (human-verified materialization lives in 7.4). **Shipped 2026-09-12** archive `openspec/changes/archive/2026-09-12-automated-trace-backflow` | S |
| 7.2e | Evaluation Report Dashboard ✅ | Evaluation & Testing | Automated evaluation reports over the existing evaluation tables (零 schema 变更，全部按需聚合)。**Backend**: `GET /api/evaluation/reports/{overview,trends,distributions,breakdowns,sessions}` — overview 四卡（质量 = offline pass_rate + online 均分；规模；覆盖度 = 活跃 dataset + 中位 items + 低样本 run 占比（<20 items，可行动 → 7.2b AI 合成）；评估错误率 `-1.0` 哨兵值单列）；trends 按 dataset/workflow/agent × day/hour（offline 走 `runs.summary`… **Shipped 2026-09-12** archive `openspec/changes/archive/2026-09-12-evaluation-report-dashboard`, `openspec/changes/archive/2026-09-12-human-annotation-trace-backflow` | M |
| 7.3 | Workflow Evaluation ✅ | Evaluation & Testing | End-to-end workflow testing + regression testing. **Shipped 2026-09-11** (archive `openspec/changes/archive/2026-09-11-workflow-evaluation/`): `AnswerSource=workflow` 端到端跑评估集（懒导入 `WorkflowExecutionService.execute`，非流式按条目执行，轨迹落 `EvaluationRunModel.trajectory`，节点级评分不在 v1）；repetitions（默认 1）输出 `pass_rate` + `consistency_rate`（Anthropic `pass@k`/`pass^k` 落地）；dataset 启动时拷贝 + content… **Shipped 2026-09-11** archive `openspec/changes/archive/2026-09-11-workflow-evaluation`, `openspec/changes/archive/2026-09-13-known-bad-exemption`, `openspec/changes/archive/2026-09-13-eval-publish-gate-versions` | M |
| 7.4 | Human Annotation ✅ | Evaluation & Testing | Human review queues for production agent outputs: reviewer worklists (`annotation_queues` + `annotation_queue_items`, unique per target, lifecycle pending → claimed → completed \ skipped), in-built CRUD + manual bulk + "from online task low-score samples" intake, three metric form types (`numeric` with min/max, `categorical`, `boolean`), suggestion-prefilled workbench reusing `trace_input`… **Shipped 2026-09-12** archive `openspec/changes/archive/2026-09-12-human-annotation-trace-backflow` | M |
| 7.4a | Human Score Calibration ✅ | Evaluation & Testing | Machine-vs-human score calibration analytics on the shared `evaluation_task_scores` ledger. **Override** is a pointer-type supersede (a new `source="human"` row carries `overrides_score_id` + structured `reason_code` + free-text `justification`; the original automated row is preserved for audit); original-row mutation is forbidden, so reconciliation stays read-side. **Shipped 2026-09-12** archive `openspec/changes/archive/2026-09-12-human-annotation-trace-backflow` | S |
...
| 4.14 ✅ (2026-09-21) | Memory Importance Scoring | Memory System | **Delivered (change `memory-importance-fusion`, archive `2026-09-21-memory-importance-fusion`)**: importance scoring with anchor tiers (0.0–0.2 fleeting … 0.8–1.0 enduring identity) emitted by the consolidation planning prompt with `[0,1]` clamp and a neutral 0.5 default; write-time scoring only — query-time re-scoring is rejected as a design choice; each tier carries a documented behavioral… **Shipped 2026-09-21** archive `openspec/changes/archive/2026-09-21-memory-importance-fusion` \| real-embedding write path (`embedding_real` flag on `memories` / `knowledge_memories`), consolidation writing-point refresh of `last_confirmed_at`, value-score observability fields, L3/L4 input path real embedding (off when the model is unavailable) | M |
| 4.15 ✅ (2026-09-21) | Multi-Signal Fusion Retrieval | Memory System | **Delivered (change `memory-importance-fusion`, archive `2026-09-21-memory-importance-fusion`)**: relevance-dominated retrieval with bounded multiplicative bias (`MEMORY_FUSION_BIAS_ENABLED`, default off — `final = relevance × decay_mult × importance_mult` clamped to a 0.3× product floor) over L3 cosine and L4 hybrid relevance normalized to one `[0,1]` scale; per-signal `breakdown` carried in… **Shipped 2026-09-21** archive `openspec/changes/archive/2026-09-21-memory-importance-fusion` | consolidation value score (confirmation freshness × log-compressed access heat; written as observability by 4.14/4.15, and since 4.25 also the capacity-eviction score — the one sanctioned consumer outside ranking); `memory_access_sessions` distinct-session counter; `last_confirmed_at` exogenous decay anchor; cross-layer normalized merge in `BuiltinMemoryProvider.search_memories` |
| 4.16 ✅ (2026-09-24) | LLM-Managed Memory | Memory System | Umbrella for agent-driven memory management. **Delivered (change `agent-memory-tools`, archive `2026-09-19-agent-memory-tools`)**: the memory tool layer — 8 built-in tools (`memory_replace`/`memory_insert`/`memory_rethink`/`memory_search`/`memory_add`/`memory_update`/`memory_forget`/`conversation_search`) seeded via the builtin registry behind `MEMORY_TOOLS_ENABLED`; tiered `MemoryProvider`… **Shipped 2026-09-24** archive `openspec/changes/archive/2026-09-19-agent-memory-tools`, `openspec/changes/archive/2026-09-24-memory-lifecycle-governance` | MemGPT/Letta (function-driven memory), deer-flow/openjiuwen (memory_* tool naming), openjiuwen `MemoryProvider` ABC (contract shape) |
| 4.17 ✅ (2026-09-20) | Memory Pressure Alert | Memory System | **Delivered (change `memory-consolidation`, archive `2026-09-21-memory-consolidation`)**: `MemoryPressureNudgeProcessor` in the 4.13 chain (registered `memory_pressure_nudge`, threshold default 0.9 — strictly above `budget_warn`, fail-fast cross-validation) injects one latched `[memory_pressure]` directive per crossing carrying usage numbers + an explicit persist-to-memory instruction, positioned… **Shipped 2026-09-20** archive `openspec/changes/archive/2026-09-21-memory-consolidation` | Anthropic context editing (`trigger/keep/exclude` vocabulary), OpenClaw pre-compaction memoryFlush (silent-turn variant deferred to v2), Hermes-agent fail-loud "Consolidate now" directive |
| 4.25 ✅ (2026-09-24) | Layered Memory System | Memory System | **Delivered (change `memory-lifecycle-governance`, archive `2026-09-24-memory-lifecycle-governance`) — rescoped from the stale three-layer framing (those layers shipped as L1-L4/Qdrant)**: memory flush + lifecycle. Flush = `pre_compaction` trigger on the 4.5 trigger bus: best-effort registration at the compaction boundary (`consolidation_flush_windows`, `MEMORY_FLUSH_ENABLED`), at-least-once… **Shipped 2026-09-24** archive `openspec/changes/archive/2026-09-24-memory-lifecycle-governance` | OpenClaw pre-compaction memoryFlush (async-handoff shape), deer-flow memory_flush_hook + three-signal eviction, Codex two-phase consolidation pipeline, Bedrock AgentCore eventExpiryDuration |
| 4.21 ✅ (2026-09-22) | Task Memory | Memory System | **Delivered (change `memory-storage-family`, archive `2026-09-23-memory-storage-family`)**: `episodes` table + `reflections` list `ReflectionEngine` (extract → score/dedupe → plan/judge → apply) sharing the consolidation engine's `ExtractFn/PlanFn/ScanFn/EmbedFn` seam; `work_context_nodes` + `work_context_edges` materialize approved reflections as a typed graph (5 node types `method / outcome /… **Shipped 2026-09-22** archive `openspec/changes/archive/2026-09-23-memory-storage-family` | Perplexity Brain "Work Memory Graph" (公开材料中**未披露**,Hecate 自行独立路径选择 ADR-024 §6 schema) |
...
| 1.3.6f ✅ (2026-09-15) | Self-Evolution Skill Loop | Agent Runtime | Closed loop over knowledge-only learned skills: quality signals (score threshold / user correction) harvest completed conversations → rule pre-filter + LLM attribution (AgentRx taxonomy) → candidate skills (procedure + guardrails partitions, structured deltas, theme merge, content-scan blocked path, knowledge-only enforcement) → four-check validation gate (dataset regression, golden-subset… **Shipped 2026-09-15**. | AgentCore Optimization (recommendation→A/B→promote), Salesforce (frozen-weight loop, "no persistence without verification"), ACE (structured deltas), GEPA (reflective attribution), Manus (approval-gated project learning), Warp×Claude (scheduler-driven improver) |
...
| 1.3.20 ✅ (2026-09-14) | Agent Versioning & Channel Publishing | Agent Runtime | Agent-level versioning + channel binding, analogous to workflow versioning (1.1.9 ✅). Each agent has a version list with snapshot commit action; a channel (API / IM) is bound to a specific version snapshot via `bind_mode` (`published` tracks latest publish; `pinned` locks a version), so runtime calls always invoke the locked configuration (immutable contract from the end-user perspective). **Shipped 2026-09-14** archive `openspec/changes/archive/2026-09-14-agent-versioning` | AgentArts (提交版本并分享 lifecycle), Salesforce (Versioned Apex, version-bound deployment), LangGraph (thread versioning per deployment), OMA v1.14 (artifact versioning) |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 4.13 | Context Engine Processor Chain ✅ | Memory System | Evolve ContextEngine from fixed 3-method ABC (select/compress/estimate) to pluggable processor pipeline. **Chain supplement (delivered, change `context-processor-chain`, 2026-09-16)**: ``ContextProcessorChain`` (``runtime/context_processors.py``) with policy resolution (``runtime/context_policy.py``) — ordered, budget-satisfied projection over atomic tool-call units (pairing-invariant healing);… archive `openspec/changes/archive/2026-09-19-durable-compaction-surface-replacement` | openJiuwen (ContextProcessor chain), AgentScope (ContextConfig + Offloader), Claude Code (5-level compression cascade), dsh CompactionEngine + surface replacement (source-verified), deer-flow TokenBudgetMiddleware |
...
| 2.6a | Multi-Agent Central Controller ✅ | Multi-Agent Orchestration | Shipped in `intent-recognition`: CONTROLLER node (per-turn recognition via the 6.23 engine, category→sub-workflow mapping, start/default/end workflow assignment, global intent with drift re-routing, isolated sub-workflow dispatch, CONTROLLER_ROUTED events for visualization). | AgentArts (multi-agent controller) |

### Canvas UI Orchestration

| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 1.1.21 | Multi-Agent Controller Canvas ✅ | Agent Development | Shipped in `intent-recognition`: CONTROLLER node with intent-package picker (published versions + pin, defaults to latest published), mapping editor rendered from the selected version's categories (removed categories flagged for reassignment), start/default/end selectors, global-intent panel, and teal intent-mapping edges visually distinct from default edges. | AgentArts (multi-agent controller), Huawei Pangu (controller) |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 4.5 ✅ (2026-09-20) | Sleep-time Memory Consolidation | Memory System | **Delivered (change `memory-consolidation`, archive `2026-09-21-memory-consolidation`)**: plan-then-apply engine — per-unit bounded LLM (extract candidates → typed operations ADD/UPDATE/SUPERSEDE/NOOP/UPDATE_BLOCK) applied by deterministic code through the shared service-layer write path (revision guards + `memory_edit_log` with `tool_name=consolidation`); SUPERSEDE lineage (`superseded_by`… **Shipped 2026-09-20** archive `openspec/changes/archive/2026-09-21-memory-consolidation` | ADR-024 KM3 (design), Letta sleep-time compute, Perplexity Brain overnight synthesis, OpenJiuwen Dreaming (checkpointed sweep + shared write path) |
| 4.18 ✅ (2026-09-19) | Conversation Recall Storage | Memory System | **Delivered (change `agent-memory-tools`, archive `2026-09-19-agent-memory-tools`)**: transcript-level recall layer — `recall_messages` metadata table + Qdrant `hecate_recall` collection, background indexer projecting `CHANNEL_WRITE` (messages-channel) events with per-session version watermarks (idempotent, self-healing), workspace/agent-scoped with Qdrant payload filtering; `conversation_search`… **Shipped 2026-09-19** archive `openspec/changes/archive/2026-09-19-agent-memory-tools` | Letta (`conversation_search` hybrid signature), OpenClaw (write-time transcript FTS + dual-lane), cubeplex (hybrid RRF + background indexer), AWS namespace scoping |
| 4.19 ✅ (2026-09-19) | Self-Editing Memory | Memory System | **Delivered (change `agent-memory-tools`, archive `2026-09-19-agent-memory-tools`)**: L1 block editors with layered semantics — `memory_replace` (exact match once; ambiguity → structured error; empty new_string deletes), `memory_insert` (line insertion), `memory_rethink` (whole-block rewrite, no auto-create); L3/L4 corrections via `memory_update`/`memory_forget` with `revision` optimistic… **Shipped 2026-09-19** archive `openspec/changes/archive/2026-09-19-agent-memory-tools` | Letta (replace/insert/rethink semantics), Hermes-agent (old_text ambiguity refusal), deer-flow (fact-level revision optimistic concurrency), OpenClaw (audit trail + immutable provenance), Codex (git-based memory audit) |
| 4.20 ✅ (2026-09-19, rescoped) | Retrieval Escalation (was Multi-Step Memory Retrieval) | Memory System | **Rescoped during 2026-09 field research**: the MemGPT heartbeat reference is retired (removed in Letta's own successor; no 2026 platform ships an in-turn forced retrieval loop). Delivered as **retrieval escalation gating** (change `agent-memory-tools`, archive `2026-09-19-agent-memory-tools`): weak/empty memory or conversation searches set a low-signal marker in the tool loop; a chain-tail… **Shipped 2026-09-19** archive `openspec/changes/archive/2026-09-19-agent-memory-tools` | OpenClaw Active Memory (escalate gating — replaces heartbeat), Hermes-agent (`exclude_session_ids` iteration), deer-flow (token-budget/anti-thrash middleware shape) |
| 4.22 ✅ (2026-09-22) | Tool Memory | Memory System | **Delivered (change `memory-storage-family`, archive `2026-09-23-memory-storage-family`)**: same store as 4.21 — tool calls land as `TOOL` events in the parent episode's `actions`; `ReflectionEngine` analyzes `actions` + `outcomes` together when extracting typed reflections, so the tool experience is implicitly captured without a second storage layer. **Shipped 2026-09-22** archive `openspec/changes/archive/2026-09-23-memory-storage-family` | AWS Bedrock AgentCore EpisodicMemoryStrategy (tool event as TOOL event) |
| 4.23 ✅ (2026-09-22) | Cross-Thread Memory Store | Memory System | **Delivered (change `memory-storage-family`, archive `2026-09-23-memory-storage-family`)**: extends existing two-layer namespace (`workspace_id + user_id`) on `memories` + `knowledge_memories` to four layers `workspace_id + team_id + actor_id + session_id`. **Shipped 2026-09-22** archive `openspec/changes/archive/2026-09-23-memory-storage-family` | AWS Bedrock AgentCore namespace (`actorId` + `sessionId` + IAM-style namespace keys) |
...
| 5.9c ✅ (2026-09-23) | Skill Auto-Detection | Tools & Plugins | Skills become discoverable without manual agent-skill association: an L1-catalog discovery pool of eligible workspace + bundled skills (`model_invocable`, non-`user` provider, plugin-enabled, precedence-collapsed, workspace trust-floor filtered) is injected alongside bound/`auto_load` entries, and the `load_skill` advertised set widens to bound ∪ discovery pool. **Shipped 2026-09-23** archive `openspec/changes/archive/2026-09-24-skill-auto-detection` | Claude Code Skills 2.0 (Jan 2026) |
...
| 5.9d ✅ (2026-09-21) | Skill Versioning | Tools & Plugins | Immutable skill snapshots with full lifecycle: commit / list / get / diff / rollback / delete, plus drift detection. Snapshots freeze seven content fields (name / instructions / allowed_tools / scripts / references / description / max_tokens); the 5-field content hash matches agent-version reference manifests, so the two stay comparable. **Shipped 2026-09-21** archive `openspec/changes/archive/2026-09-21-skill-versioning` | Claude Code Skills (version tracking), OpenClaw (ClawHub versioning) |
| 5.9e ✅ (2026-09-22) | Skill Dependency Declaration | Tools & Plugins | SKILL.md frontmatter `requires` (list of `{name, provider?}`) declares inter-skill dependencies; `plugin.json` namespace `extensions.io.github.xueyufish.requires` carries the same shape at plugin level. Authoring-time graph validation: missing dependency / DFS cycle detection / cross-source rejection (`user`/`project`/`bundled`/`plugin` allow matrix) / trust-tier anti-escalation, all returning… **Shipped 2026-09-22** archive `openspec/changes/archive/2026-09-22-skill-dependency-declaration` | npm (dependency resolution), Helm (Chart.lock: declare loose, lock at build), Claude Code plugin dependencies (install-time resolve, load-time check) |
...
| 6.47 ✅ (2026-09-14) | Model Service Publishing | Model Management | Wire the published/unpublished lifecycle (already engineered as 6.45 Model Lifecycle Manager ✅ staging channels + promotion gates) into the provider/model management surface: `model_registry` gains a publish state (draft → testing → published); unpublished models are hidden from the application-facing reference surface (`GET /v1/models` → Create Agent Model dropdown, agent `llm_config` binding)… **Shipped 2026-09-14**. | AgentArts (model service publish lifecycle), 6.45 Model Lifecycle Manager ✅ |
| 6.48 ✅ (2026-09-14) | Model Management Quick Wins | Model Management | Two low-cost items pulled forward from the AgentArts comparison (pure frontend + one aggregation query, no schema change): (1) list-level search/filter for providers and models on settings/models (AgentArts has keyword search on both levels; Hecate has none); (2) provider card call-count wiring — aggregate invocation counts from existing traces/costs data into the provider table (AgentArts… **Shipped 2026-09-14**. | AgentArts (provider card stats, list search) |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 10.3b | SCIM Directory Sync ✅ | Multi-Tenant & Enterprise | User/group auto-provisioning via SCIM 2.0 protocol. Syncs from Azure AD, Okta, OneLogin to Hecate org/workspace membership. | SCIM RFC 7643/7644, Azure AD SCIM, Okta SCIM |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 6.23 | 5-Level Intent Recognition ✅ | Agent Intelligence | Shipped in `intent-recognition` (merges 1.3.10; L5 meta intent delivered as a deterministic policy boundary, not a recognizer — hooks/approval enforce gated routes regardless of recognition output). Atomic intent (decision cache → package patterns → few-shot → LLM fallback, recognition model configurable separately from execution model) → workflow intent (multi-turn accumulation) → session intent… | AgentArts AgentStudio (formerly Versatile) |
| 6.49 | Intent Package Asset ✅ | Agent Intelligence | Intent packages as manageable data assets, paired with 6.23's recognition engine: each package = named set of intent categories + sample utterances per category (bulk entry CSV/JSON, import/export), referenced by the workflow advanced intent recognition node and controller intent routing (2.6a/6.23) as few-shot classification evidence to lift accuracy. | AgentArts (意图包/意图管理), 7.2 evaluation dataset machinery ✅ |
...
| 5.5d ✅ | Dual-Format Plugin Convergence & Export | Tools & Plugins | One Agent Plugins 1.0 package is simultaneously a conformant open-standard plugin (skills/mcp.json visible to all clients) and a Hecate deep-integration plugin. **Namespace resolution (closed 2026-09-19)**: the `io.hecate` placeholder is dropped — that domain is registered by a third party; the namespace is the code constant `AGENT_PLUGIN_NAMESPACE = "io.github.xueyufish"` in… archive `openspec/changes/archive/2026-09-19-dual-format-plugin-convergence` | Agent Plugins 1.0 spec §Client Extensions (§5.6 extensions + §8.2 extension directories), npm/OCI "interchange envelope" pattern, Claude Code marketplace (git-repo format) |
...
| 1.3.21 ✅ (2026-09-09) | Engine Parity: Dynamic Fan-out, Time-Travel Resume, Declarative Interrupts (NEW — 2026-09-07 deer-flow/LangGraph engine comparison) | Agent Runtime | Close the three engine-level gaps found by diffing Hecate's Pregel against LangGraph (deer-flow's inherited engine; source-verified). Context: deer-flow itself uses almost none of these — its v1 research loop is sequential ``Command(goto)`` cycles with zero ``Send()`` fan-out, and v2 retreats to a single-agent + middleware harness — so the gaps are **vs the engine it inherits**, not vs the… **Shipped 2026-09-09** archive `openspec/changes/archive/2026-09-08-declarative-interrupts`, `openspec/changes/archive/2026-09-09-node-cache-policy`, `openspec/changes/archive/2026-09-09-time-travel-resume`, `openspec/changes/archive/2026-09-09-dynamic-fanout` **Deferred** (防遗忘): provider prefix-cache usage passthrough, distributed cache backend, single_flight, large-payload offloader, fork cache inheritance; FORK payload compression, dangling-turn sweep, commit-point enrichment, fork quotas; platform-quota linkage for FanoutLimitError | LangGraph Send / Command / interrupt / get_state_history / update_state / CachePolicy (docs.langchain.com, source-verified), deer-flow v1 sequential research loop + v2 lead-agent retreat (source-verified), Hecate engine audit 2026-09-07 |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 4.24 ✅ (2026-09-22) | Memory Versioning | Memory System | **Delivered (change `memory-storage-family`, archive `2026-09-23-memory-storage-family`)**: monotonic `revision` counter on every L1 / L3 / L4 row (incremented on every successful mutation); `memory_update` / `memory_forget` accept optional `expected_revision` — mismatch returns structured revision-conflict error carrying the current revision. **Shipped 2026-09-22** archive `openspec/changes/archive/2026-09-23-memory-storage-family` | Mem0 (revision+supersede lineage), Letta (version history) |
...
3. **Graph/State Machine as a Widespread Execution Model**: graph-based execution is a common backend choice (Agentforce's Agent Graph, ADK 2.0's WorkflowRuntime, watsonx's Agentic Workflow, Copilot Studio's Topic), but NOT universal — agent loops, code-execution harnesses, and external coding agents coexist as first-class execution modes. Hecate's PregelRuntime is one strong backend, not the only intended shape.

4. **LLM Called Only Where Necessary**: Enterprise platforms minimize LLM invocations. Agentforce has 8 explicit LLM call points; everything else is deterministic. Vertex AI's WorkflowAgent contains zero LLM. Hecate's `_LLMWorker` is the runtime-graph LLM call point; Condition/Tool/Variable/Knowledge workers are deterministic. Suggestion generation (1.3.8) is LLM-backed via SuggestionService outside the worker set — the earlier "sole LLM call point" claim was wrong.
