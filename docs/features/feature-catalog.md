...
> **Status**: P1 complete; P2 complete; P3 complete (69/69) — all four close-out items delivered (9.1a + 9.2 / 6.27 / 5.4b); P4 in progress; P5 pending.
...
| 1.3.2 | Tool Calling ✅ | Agent Runtime | Agent calls tools with parallel execution, retry, and timeout support. When the assistant proposes multiple tool calls in one turn, ``ToolWorker.execute`` and the chat path-A loop dispatch them concurrently via ``asyncio.gather``; result ordering matches the LLM's submission order regardless of completion order. Each call still flows through the full per-tool pipeline (access policy → approval callback → middleware chain → port execution → post hook) so concurrency is per-tool, not per-batch. Engine-level retry is per-tool via ``RetryStrategy.with_config`` (1.3.5); per-tool timeout is honoured via the ``AgentTool`` / LLM-provider ``asyncio.wait_for`` seam — hung tools do not block sibling calls. | Claude Code (40+ tools), OpenClaw (TypeBox) |
...
| 1.3.4 | Human Intervention ✅ | Agent Runtime | Execution pause, human approval, result correction, resume continue. **Result correction wiring (delivered)**: the Pregel runtime writes the human-supplied resume value into the ``_resume_value`` channel on restore (``_restore_from_checkpoint``); ``LLMWorker.execute`` and ``execute_stream`` consume it via ``_consume_resume_value`` and materialize it as user-role messages appended to the conversation, then clear the channel via ``_resume_value = None`` in ``channel_updates`` so it is not re-injected on subsequent turns. Accepted shapes: ``str`` → ``{"role": "user", "content": <str>}``; ``{"role": ..., "content": ...}`` → single message; ``{"messages": [...]}`` / list of message dicts → appended verbatim. Failure modes (PreLLMHook BLOCK, PostLLMHook BLOCK) also clear the resume value. **Planned enhancement (Sprint 7)**: Fail-closed approval — no answerer or throwing answerer → deny (dsh `'unavailable'` semantics); only explicit allowed-once grants; `ask`/`never` policy state machine as durable config; `approval/asked` + `approval/decided` audit pair as EventStore events, turn-enclosed. Sequenced after 1.3.19 (audit pairs consume the enriched event log). **Seam notes**: 增量加 `APPROVAL_ASKED`/`APPROVAL_DECIDED` EventType 枚举值（未知类型回读回落 `CUSTOM` 已保兼容）；审计对须 turn-enclosed（`TURN_START`/`TURN_END` 边界）；复用 1.3.19 的 B2 日志推导恢复路径（见 [ADR-030 下游消费者接缝登记表](../design/adr/030-event-sourced-execution-state.md)）。 **已交付 ✅**（PR #86 + #87，`openspec/changes/archive/2026-08-21-guardrail-upgrade-trio/`） | Bisheng (workflow pause), LangGraph (interrupt), dsh approval seam (source-verified), MAF durable approvals, AgentScope PermissionEngine |
| 1.3.5 | Error Recovery ✅ | Agent Runtime | Tool failure retry, model degradation, graceful degradation | OpenClaw (14 failure types) |
| 3.1.1 | Document Parsing ✅ | Knowledge Base/RAG | PDF/Word/PPT/Excel/HTML/Markdown parsing. **Office Open XML supplement (P1 audit, this branch)**: `.pptx` text extraction via stdlib zip+XML over DrawingML `<a:t>` runs (slides ordered numerically); `.xlsx` via shared-strings + worksheet XML resolution (shared `t="s"`, inline `t="inlineStr"`); corrupt archives raise `ValueError`; legacy binary `.ppt`/`.xls` remain unsupported (convert to OOXML first). PDF (pdfplumber→PyPDF2), DOCX (python-docx), HTML (bs4), Markdown/text unchanged. | Docling (20+ formats), RAGFlow DeepDoc |
| 3.2.1 | Vector Search ✅ | Knowledge Base/RAG | Embedding + ANN search | LlamaIndex, Qdrant |
| 3.2.6 | Chunking Strategy ✅ | Knowledge Base/RAG | Auto-chunking, character-based, separator-based, semantic chunking. **Strategy supplement (P1 audit, this branch)**: `ChunkingStrategy` enum on `TextChunker` — `character` (default, legacy behavior preserved), `separator` (blank-line/structural split, fragments merged up to chunk_size, oversize falls back to character), `auto` (per-document detection: blank-line paragraphs or markdown headers → separator, else character), `semantic` (sentence-level packing with embedding-similarity breakpoints via an explicitly injected `EmbeddingService`; sync `encode` supported inline, awaitable via `chunk_text_async`; missing service falls back to separator with a warning — no implicit model loading). `KnowledgeBaseService.ingest_document[_text]` accept `chunking_strategy=`. Fixed a forward-progress bug: `chunk_overlap >= chunk_size` previously moved `start` backwards and hung forever. | Coze (4 strategies), LlamaIndex |
...
| 13.2 | Self-Hosted Deployment ✅ | Deployment & Operations | Docker Compose / K8s one-click deployment. **K8s supplement (P1 audit, this branch)**: `deploy/k8s/` — namespace + PostgreSQL 16 / Qdrant / MinIO / Temporal Deployments+Services+PVCs, an `alembic upgrade head` migration Job, and the app Deployment with readiness probe (`/health/ready`) and compose-matching env wiring (`DATABASE_URL`/`QDRANT_URL`/`MINIO_URL`/`TEMPORAL_SERVER_URL`); see `deploy/k8s/README.md` for apply order and secret guidance. Compose path unchanged (`docker/docker-compose.yml`, blue-green variant included). | Dify, Bisheng |
...
| 1.1.2 | Visual Workflow Canvas ✅ | Agent Development | Drag-and-drop DAG editor with node connections, conditional branching, parallelism. **Enhancement TODO (→ P4)**: integrate ConfigPanel into right-side panel; persist node layout to localStorage | Coze (DAG), Langflow (React Flow) |
| 1.1.3 | Workflow Node Type Library ✅ | Agent Development | LLM, Code, Condition, Tool, Knowledge Base Retrieval, Variable, Batch Processing nodes. **Enhancement TODO (→ P4)**: add fan-out/merge node creation and editing (currently visual-only, no palette/config) | Coze (8 types), Dify |
...
| 6.7 | Model Playground ✅ | Model Management | Built-in model test UI for connectivity and response quality debugging, parameter tuning. Debug page at settings/models/debug with model selector, parameter tuning (temperature, max_tokens), streaming display, latency/usage visualization, test history, and error suggestions. **Enhancement (AgentArts 形态对齐, 2026-09-06)**: dual-model side-by-side comparison mode — send one prompt to models A and B simultaneously, render responses in parallel columns (pure frontend over two invoke calls; complements 6.8a which A/B-tests production traffic, not interactive debugging). AgentArts 调测页 also spans 4 model types (chat/vision/embedding/rerank) — Hecate 6.11 classification ✅ covers type filtering. | AgentArts (model debugging, A/B 对比调测) |
...
| 4.8 | Evidence Tracker ✅ | Memory System | Captures tool execution results with provenance tracking, importance scoring, and re-reference boosting. **Tracker supplement (P2 audit, this branch)**: ``EvidenceTracker`` (`runtime/evidence.py`) — one instance per run, injected into the execution context by ``PregelRuntime``; ``ToolWorker`` captures each tool outcome (success, execution error, access denial, approval rejection) with provenance (node_id, superstep) and importance scoring (base 0.5; +0.2 re-reference boost when an equivalent call repeats via ``match_existing``; +0.1 error; +0.1 explicit reuse). After the run, ``WorkflowExecutionService`` persists the snapshot as ``EvidenceModel`` rows (best-effort); production ``RuntimePort.evidence_query`` queries persisted records by session with a min-importance filter. | AgentArts (evidence tracking) |
| 4.9 | Task Phase Detection ✅ | Memory System | Detects task phases (EXPLORE/CONVERGE/EXECUTE/VERIFY) for dynamic tool and context filtering. **Detector supplement (P2 audit, this branch)**: ``detect_task_phase`` (`runtime/task_phase.py`) — deterministic keyword heuristics over the recent window plus failed-tool-result detection (errors force VERIFY) and a channel-state override (``_task_phase``); wired into ``LLMWorker`` and published as ``task_phase`` on the LLM span attributes, the ``LLM_REQUEST`` event payload, and the tool-gate context so ``available_when`` expressions can gate tools per phase. | AgentArts AgentBase (formerly Versatile) (phase detection) |
| 4.10 | Token Budget Governance ✅ | Memory System | Per-session token budget tracking with three-level degradation (DROP/COMPRESS/EMERGENCY) and budget snapshot persistence. **Ladder supplement (P2 audit, this branch)**: the LLMWorker context pipeline now formalizes the ladder — DROP (message selection), COMPRESS (engine compression), EMERGENCY (hard truncation keeping system messages and newest content) — and emits a fold-skipped ``BUDGET_SNAPSHOT`` CUSTOM event (budget, tokens/messages before/after, levels applied) to the EventStore whenever degradation fires, making the trail queryable from the execution log. | Claude Code (5-level compression) |
| 4.11 | Provider-Shaped Context ✅ | Memory System | Provider-specific context shaping strategies (OpenAI, Anthropic, Default) with automatic model prefix detection. **Strategy supplement (P2 audit, this branch)**: ``runtime/context_shaping.py`` — ``ProviderShapingStrategy`` with ``AnthropicShapingStrategy`` (system-message consolidation into a single block, empty-content drop, tool_calls anchors preserved), ``OpenAIShapingStrategy`` (orphan tool-message drop with pending-window invalidation on intervening turns), ``DefaultShapingStrategy``; prefix-based resolution (``claude`` → Anthropic, with ``provider/model`` prefix unwrapping). Applied in the production ``RuntimePort.context_assemble`` so every LLM call receives a provider-acceptable conversation shape. | AgentArts (provider adaptation) |
| 4.12 | Message Prioritization ✅ | Memory System | Prioritizes messages by importance for context window optimization. **Engine supplement (P2 audit, this branch)**: ``PriorityContextEngine`` (`runtime/context.py`) — importance scoring (recency + role bonuses; failed/oversized tool results penalized) with guaranteed retention of system messages and the newest user message, budget-fit selection preserving conversation order, and scored top-half compression; now the production engine in ``WorkflowExecutionService`` (replacing the recency-only ``InMemoryContextEngine``). | OpenClaw (Context Engine) |
...
| 13.3 | Offline Deployment ✅ | Deployment & Operations | Air-gapped / offline deployment for regulated environments (government, defense, finance). **Bundle supplement (P2 audit, this branch)**: `deploy/offline/` — `bundle.sh` (docker-saves the platform + infra images, downloads dependency wheels, exports pulled Ollama model blobs, writes a SHA256SUMS manifest, optional `--registry` retag/push for an internal mirror), `load.sh` (checksum-verified `docker load` + model restore, idempotent), `docker-compose.offline.yml` overlay (adds the bundled Ollama runtime, pins the pre-built `hecate-app:offline` image via `build: !reset null`, re-asserts empty `OTEL_EXPORTER_OTLP_ENDPOINT` — Hecate has no other phone-home), and `docs/how-to/deploy-offline.md` operator workflow. Distinct from 13.2 (Self-Hosted) which assumes internet access for image pulls and model API calls. | Palantir Apollo (air-gapped SaaS via cryptographically signed bundles), Bedrock AgentCore (VPC-only MicroVM), openJiuwen (offline installer), 华为 AgentArts (private deployment) |
| 14.2 | Webhook Callbacks ✅ | Deployment & Operations | Outbound webhook callbacks for platform events (agent completed, workflow finished, tool error, threshold alert). Configurable per-workspace webhook endpoints with HMAC-SHA256 signatures, retry with exponential backoff, dead-letter queue for failed deliveries. Complements 8.7 SIEM Pipeline (inbound security events) with outbound automation triggers. Foundation for 11.x channel push notifications and 13.9 Scheduled Tasks result delivery. **Enhancement note (AgentArts 对齐, 2026-09-06, low-priority/on-demand)**: message-template asset layer — predefined JSON message-body templates (exception/custom types, visibility scope, import/export) selectable by workflow error nodes for exception notification. Hecate's webhook/alert channels cover delivery; only the template-as-asset packaging is absent — add when workflow exception-notification demand materializes. | Dify (webhook node), Slack/Stripe webhook patterns (HMAC signatures), openJiuwen (event subscription), AgentArts (消息模板) |
| 3.1.7 | Multi-Vector-DB Support ✅ | Knowledge Base/RAG | Pluggable vector database backends: Qdrant, Milvus, Weaviate, Chroma; unified vector operations interface with per-collection backend selection. **Backend supplement (P2 audit, this branch)**: ``MilvusVectorStore`` (pymilvus ``MilvusClient``: explicit VARCHAR-PK schema, COSINE AUTOINDEX, workspace_id filtering; lazy SDK import with mock fallback) and ``WeaviateVectorStore`` (weaviate-client v4: ``Vectorizer.none()`` collections, ``workspace_id`` property filtering, http/gRPC URL parsing) added to ``factory.py`` with ``MILVUS_URI``/``MILVUS_TOKEN``/``WEAVIATE_URL``/``WEAVIATE_GRPC_HOST``/``WEAVIATE_API_KEY`` settings and ``pymilvus``/``weaviate-client`` in the ``rag`` extra. Sparse vectors: Milvus/Weaviate adapters inherit the ABC's application-layer RRF fusion (Qdrant keeps native hybrid). | openJiuwen (4 vector DBs), LlamaIndex (vector store abstraction) |
...
| 10.2 | RBAC ✅ | Multi-Tenant & Enterprise | Workspace-level role-based access control (admin/editor/viewer) with FastAPI dependency guards. Row restored 2026-09-07 P2 audit — implementation (`core/deps_workspace.py`, role hierarchy + workspace membership checks) and `test_rbac_api.py` / `test_groups.py` coverage were present, but the row itself had been dropped in the 2026-08-22 reclassification while remaining referenced by 5.5c ("Deferred to 10.2 RBAC") and the P2 dependency chain | Bisheng (deep RBAC + user groups) |
...
| 9.1a | Output Injection Type Detection ✅ | Security & Compliance | Detect downstream-system injection patterns in LLM output (9.1a): code (Python `exec`/`eval`/`__import__`/`compile`), SQL (`DROP TABLE`/`UNION SELECT`/`;DELETE`), template (Jinja SSTI `{{ config }}`/`{% import %}`), XSS (`<script>`/`onerror=`/`javascript:`). Regex recognizer registry mirroring `DLPRecognizer` shape; per-type action (`audit | block | mask | sanitize`, default `audit`); custom patterns supported via `guardrail_config["injection_detection"]["custom_patterns"]`. Findings persisted to `SecurityFindingModel` via `SecurityFindingWriter`; `EventType.INJECTION_DETECTED` emitted. Bedrock Guardrails Standard tier "code elements" correspondence. | Bedrock Guardrails Standard tier, DeerFlow SkillScan, OWASP LLM01:2025 Scenario #5 |
| 9.2 | System Prompt Leakage Protection ✅ | Security & Compliance | Detect and block system prompt content exfiltration (OWASP LLM07:2025) via winnowing n-gram fingerprint matching (n=5, window=4, blake2b). Threshold default 0.20 (>20% baseline overlap). Severity tiers: persona LOW / roles HIGH / rules HIGH / secrets CRITICAL. Default action `block`; `sanitize` redacts matched windows with `<REDACTED>` markers. Prevents prompt injection attacks that attempt to extract the system prompt via "repeat your instructions" or "what are your rules" queries. Findings persisted; `EventType.PROMPT_LEAKAGE_DETECTED` emitted. Embedding-based semantic similarity v2 reserved for future change. | Lakera Red, Promptfoo, Amazon Bedrock Guardrails, OWASP LLM07:2025 |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 9.4 | Execution Security ✅ | Security & Compliance | Tool call approval, code sandbox isolation, access control, four-level risk authorization (LOW/MEDIUM/HIGH/CRITICAL, supports once/session/project/global scope). **Planned enhancement (Sprint 7)**: Content-aware gating — bash pipeline static analysis (AgentScope `_bash_parser` pattern) for permission decisions beyond risk_level; monotonic denial invariant — guards can only deny, no listener ordering can resurrect a denied call (dsh ToolLayer.guardReason). Sequenced after 1.3.19. **Seam notes**: 注册单调拒绝 invariant 进 LogInvariants 运行时校验层；复用 `CHANNEL_WRITE_REJECTED` 的"被拒=审计事件"范式（fold 跳过、审计保留）。 **已交付 ✅**（PR #86 + #87，`openspec/changes/archive/2026-08-21-guardrail-upgrade-trio/`） | OpenClaw (exec approval + sandbox), RelayAgent (four-level risk authorization), AgentScope PermissionEngine (bash static analysis), dsh monotonic guards (source-verified), Claude Code auto mode |
...
| 9.2 | System Prompt Leakage Protection ✅ | Security & Compliance | Detect and block system-prompt / instruction exfiltration attempts in LLM output (OWASP LLM07) — output-security leg complementing 9.1a's output-side SS3 detection (both inspect LLM responses before they flow to downstream systems or surface to users). **Delivered ✅** (change `output-side-typed-findings`, 2026-08-22; supersedes P3 close-out placeholder). Depends on: Output Security ✅. | Lakera Red, Promptfoo, OpenClaw |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 5.4b ✅ | MCP Streamable HTTP Transport (2026-07-28 spec) | Tools & Plugins | Server side speaks MCP protocol `2026-07-28` (stateless core, no `initialize` handshake, header-based routing) on a single `/mcp` endpoint. Client side uses the v2 `mcp.Client` API with automatic protocol-era negotiation (prefers `2026-07-28`, falls back to the legacy `initialize` handshake for older servers). Dependencies: `fastmcp>=4.0.0b3,<5` + `mcp>=2.0.0` + `mcp-types>=2.0.0`. Hecate does not test or guarantee behavior on the deprecated 2025-03-26 / 2024-11-05 protocol eras (alpha software, no commercial deployment to break). Side effect of the upgrade: pre-existing `/mcp` mount bug fixed (the ASGI app was constructed but never mounted); MCP server now actually responds at `/mcp`. | MCP 2026-07-28 spec (Anthropic, Linux Foundation Agentic AI Foundation), official python-sdk 2.0.0, fastmcp 4 |
...
| 5.5 ✅ | Plugin System | Tools & Plugins | **Plugin runtime engine**: `plugin.yaml` manifest loading, local directory discovery (`plugins/` scan), basic `api_version` / `min_platform_version` compatibility check, extended `PluginLifecycle` (`on_enable` / `on_disable` / `on_config_change`), `PluginModel` DB table with per-workspace enablement, `config_schema` (JSON Schema) → DB storage → runtime injection, permission declaration + enforcement, REST API (list / enable / disable / configure), frontend plugin management page (list + config form auto-generation from `config_schema`), entry loading via `python:module:Class` (in-process importlib) and `mcp://endpoint` (via existing MCP Client 5.3), MCP endpoint connection management UI. Architecture: in-process + MCP hybrid (no custom daemon — out-of-process isolation deferred to platform infrastructure / WASM in P5). Security model: permission declaration + enforcement (Dify model), signing deferred to 5.13. **T0 tightening**: loader rejects runtime-installed `python:` entries whose module is not first-party (`python:hecate.` prefix) — SaaS rejects outright, self-hosted default-deny with `PLUGIN_PYTHON_ENTRY_ALLOWLIST` (segment-boundary prefix match); install-time pre-check rolls back the extracted directory on rejection; SaaS skips runtime `uv pip install` for `requirements.txt`; operationalizes ADR-029 "any artifact acquired by installation at runtime is never T0". **Does NOT include**: 6 plugin type ABCs (TP5), Python SDK module (TP5), packaging/distribution (5.5b). **Repositioned**: `.hecate-plugin` narrowed from ecosystem-facing format to **P-tier deep-integration format** (first-party/private plugins using the 8 type ABCs; org admin install on self-hosted, operator on SaaS; T1/T2 runtime) — the ecosystem-facing third-party format is now Agent Plugins 1.0 (5.5c). | Dify (plugin daemon + YAML manifest), OpenClaw (in-process + pluginApi compat), AgentArts (UI-driven config), Salesforce (code-first + UI config dual path) |
| 5.5 (TP5) ✅ | Plugin Type Taxonomy + Developer SDK | Tools & Plugins | **8 plugin types** classified by capability: **Tool Plugin** (callable function, new ToolPluginBase), **Extension Plugin** (hook/middleware injection, new ExtensionPluginBase wrapping existing Guardrail Hooks — Google ADK BasePlugin pattern), **Trigger Plugin** (event-driven invocation: webhook/schedule/MQ, new TriggerPluginBase), **Model Plugin** (custom LLM provider, new ModelPluginBase based on existing InferenceBackendABC), **Channel Plugin** (existing ChannelBase + plugin.yaml support), **Evaluator Plugin** (existing EvaluatorBase + plugin.yaml support), **Auth Provider Plugin** (existing AuthProvider + plugin.yaml support), **Secret Provider Plugin** (existing SecretProvider + plugin.yaml support). Built-in providers continue code registration; third-party providers load via plugin.yaml. `hecate.plugin` Python SDK module for type-safe plugin development. `python -m hecate.plugin.cli init` scaffold generator. Hot-reload during development (file watcher → re-register). Full `pluginApi` install-time compatibility validation (API surface + SDK version checks). API-type plugin online creation UI (AgentArts-style form-driven). **Deferred**: Datasource Plugin (overlaps with knowledge base system), AgentStrategy Plugin (touches Worker core architecture, deferred to P4). **Depends on**: 5.5 runtime engine. **Frozen**: taxonomy frozen at 8 types — no further expansion; ADR-029's four mounting planes supersede type-based classification as the primary extension taxonomy. | Dify (6 plugin types + SDK), Google ADK (BasePlugin callback pattern), OpenClaw (30+ capability types + pluginApi compat), HermesAgent (12+ plugin types), AgentArts (UI-driven plugin creation) |
| 5.5c | Agent Plugins 1.0 Standard Ingestion | Tools & Plugins | Native ingestion of **Agent Plugins 1.0** packages (open standard by Vercel with OpenAI/Microsoft/Amazon/Cursor/GitHub; Google joined as core maintainer; shipped day-one in Codex/ChatGPT, VS Code, Cursor, Copilot, Kiro) as the **ecosystem-facing third-party plugin format**. Single adapter module (`plugin/agent_plugins.py`) — `PluginManifest` and the 8 type ABCs untouched. Install sources: directory / git URL / zip-as-transport-only. Closed-manifest validation (10 top-level fields; unknown top-level fields warn+continue, other violations fatal; unrecognized `$schema` version rejected, offline validation). Fixed-location discovery — `skills/` direct children only (no recursion), `mcp.json` at root — with **skip-and-continue** component failure semantics and path containment (symlink escape rejected). SKILL.md → SkillModel import: frontmatter `name` (must equal dir name) / `description` → existing fields; body → `instructions`; `license`/`compatibility`/`metadata`/`allowed-tools` stored as JSON; new `source` + `origin` fields with pin-by-hash; reuses existing SkillLoader token budget (progressive disclosure deferred to v2). mcp.json http/sse entries → MCPServerRegistry via existing pool/breaker/reconnect (HTTPS enforced non-loopback). **Component-level trust dispatch (ADR-029 alignment)**: skills→T4 + http/sse-MCP→T2, installable by workspace admin (`plugin:install:workspace`); **stdio entries = local subprocess code execution → org admin only on self-hosted (`plugin:install:platform`), SaaS default-deny** (config-gated, not hardcoded). Self-hosted stdio subprocesses execute inside the container sandbox pool (9.4c) — T1 semantics get T1 isolation. Also accepts bare SKILL.md directories without plugin.json (Claude Code ecosystem compatibility; hedges Anthropic's non-signature of the standard). **Depends on**: 5.5 ✅ + 5.4c ✅ + 5.9 ✅. Implementation: `plugin/agent_plugins.py` + `plugin/stdio_sandbox.py` adapter modules; `PluginModel` rows of `type="agent-plugin"` carry `origin`/`content_hash`/`scan_result` provenance; `SkillModel` rows carry `source="plugin"` + `origin` + `plugin_id` FK; SkillLoader filters by enabling plugin; MCP projection uses `<plugin>__<server>` names; stdio runs in 9.4c sandbox via docker wrapper; `AGENT_PLUGINS_INGESTION_ENABLED` default-off + `PLATFORM_PLUGIN_INSTALLERS` config allowlist; switch serves as emergency kill-switch. **Deferred to 10.2 RBAC**: `is_platform_admin` role and `plugin:install:platform` fine-grained permission string — v1 substitutes a config allowlist so this change ships without building a role system. **Not in scope**: `MCPServerModel` independent table (5.4b), MCP 2026-07-28 dialect migration (5.4b), `hecate plugin export` + `io.hecate/` namespace (5.5d), git index marketplace (12.0), `scan_result` real implementation (5.13a). | Agent Plugins 1.0 spec (agent-plugins.org), Agent Skills spec (agentskills.io, 25+ frameworks), Codex/ChatGPT/VS Code/Cursor/Copilot/Kiro (day-one support), [OpenClaw](https://github.com/openclaw/openclaw) (4-ecosystem plugin bundles, native `plugin.json` reader), [google/skills](https://github.com/google/skills) "plugins (Skills + MCP servers)" packaging precedent, [Bedraock AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/harness-skills.html) git-as-source blueprint (4 sources: curated/git/S3/path), [IBM watsonx](https://developer.watson-orchestrate.ibm.com/agent_skills/manage_skills) upload-time script deny-list, [AgentScope-Java](https://github.com/agentscope-ai/agentscope-java) four-layer skill composition |
| 5.13a ✅ | Plugin Content Scanning (split from 5.13) | Tools & Plugins | Install-time content scanning for declarative (T4) plugin content — extracted from 5.13 and pulled forward to P3 as the **precondition for shipping 5.5c**: prompt-injection pattern detection (regex + heuristics, incl. invisible-Unicode steganography per CSA Agent Context Poisoning research), secret detection, `allowed-tools`/permission-declaration audit. Fail-closed on install (block/warn/allow severity tiers; org threshold configurable); rescan on enable; results persisted + `GET /api/plugins/{id}/scan` + Ops Center display. Rationale for scanning-before-signing: for pure-data T4 content the attack surface is injection, not identity (Snyk ToxicSkills: 36.82% of ClawHub skills flawed, 13.4% critical, 76 confirmed malicious — injection present in 91% of malicious samples; ClawHavoc campaign delivered base64-obfuscated `curl\|bash` that evaded automated scanners; MCP official registry still in preview explicitly delegates scanning to downstream package registries and clients — auditing responsibility remains at the ingest layer). Cryptographic signing/digest/score remain in 5.13 (P5, marketplace-bound). **Engine form decided**: v1 = pure rule engine (regex + heuristics + invisible-Unicode pattern set per CSA research) — deterministic, auditable, zero-cost under fail-closed; LLM second-pass review deferred to v2 as optional org-configurable enhancement. **Design refined** (17+ platforms research): obfuscation layer v1 = NFKC + bounded base64/hex decode-rescan (full confusables table deferred v1.1; `transform` field reserved in finding schema); severity = file-role × rule matrix keyed to runtime exposure (frontmatter/body/mcp.json/nested/README), verdict = severity line vs `AGENT_PLUGIN_SCAN_BLOCK_AT` (default high); oversize text (>1MB/file) produces a finding instead of silent skip (22MB-padding evasion lesson); ack suppression keys on (content_hash, rule_id) so warn-tier acknowledgments survive rescans but invalidate on content change; URL detection narrowed to paste-site/IP-literal/punycode (full declared-vs-extracted reconciliation deferred). Industry alignment: the scanning camp (DeerFlow SkillScan deterministic-first + LLM second pass, Hermes-agent quarantined-bundle scan with lockfile + 4-tier trust, Dify publish-side static analysis + signature default-on, Google GE "Governing Agent Skills" char-level invisible-Unicode + injection scan deny-by-default, ClawHub VirusTotal publish-side, FortiCNAPP 24 deterministic rules with SK-* taxonomy, IBM watsonx script deny-list) converges on exactly this shape; hyperscalers otherwise substitute sandboxing (Bedrock AgentCore declares skills "trusted input, no inspection"; Manus ephemeral VM; AgentArts/openJiuwen contract-based uploader liability — note openJiuwen is Huawei AgentArts' open-source sibling, same vendor, not an independent data point; AgentScope/CatPaw runtime per-call permissions). **Depends on**: 5.5c (ingest pipeline). Implementation (`openspec/changes/archive/2026-08-18-plugin-content-scanning/`): `plugin/content_scanner.py` rule engine (16 regex rules across injection/secrets/URL + Unicode thresholds tag-run >10 / total >100 + allowed-tools audit `*`/shell/write/net + NFKC & bounded base64/hex decode-rescan with `transform` field; file-role × rule severity matrix over 6 roles; verdict = severity line vs `AGENT_PLUGIN_SCAN_BLOCK_AT`, default high; oversize text >1MB/file → finding, never silent skip; scanner crash/timeout → install rejected, evidence redacted to 8-char fingerprints); install/enable fail-closed enforcement via `ScanBlockedError` (API 422 with findings); enable-time rescan (scanner-version drift + 5.5c-era null backfill); findings projected to `SecurityFindingModel` (idempotent per (content_hash, scanner_version); install-blocked attempts per (name, origin, rule)); ack suppression keyed (content_hash, rule_id) via `POST /api/security/findings/{id}/acknowledge`; `GET /api/plugins/{id}/scan`; `AGENT_PLUGINS_INGESTION_ENABLED` default flipped on (kill-switch retained). | Snyk ToxicSkills (0% FP on top-100 legit bar), Koi Security ClawHavoc + Unit 42, Embrace the Red "Scary Agent Skills" (tag-run >10 codepoints = critical; total >100), CSA Unicode Injection in AI Skills + SKILL.md Agent Context Poisoning, DeerFlow SkillScan, Hermes-agent skills quarantine/lockfile, Dify marketplace-toolkit, Google Gemini Enterprise Governing Agent Skills, FortiCNAPP Skills Scanning (SK-*), IBM watsonx upload-time deny-list, OWASP Agentic Skills Top 10 |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 11.2 | Web Widget (Simplified) ✅ | Multi-Channel Access | Embedded web chat component — **simplified version (Wave 1, P3)** = 内部 Portal / iframe-embeddable for any Hecate deployment（直接复用现有 `(dashboard)/chat` + 员工 JWT 登录），S。架构决策：**Web Widget 不走 ChannelBase**——浏览器直接调 `/v1/chat/completions`，与 IM 渠道（webhook 推送）是不同的抽象层。详见 [ADR-031](../design/adr/031-web-widget-iframe-architecture.md)。**完整版（公开匿名场景：WidgetModel + 临时 JWT 签发 + Origin 白名单 + RS256 + JS bundle）**——Deferred to P5（按需触发：to-C / 公开网站场景，trigger = first explicit customer demand），M。 | Coze, Dify, Intercom (JWT + bundle optimization), Salesforce Enhanced Web Chat (RS256 + identityToken API), Google Dialogflow Messenger (Web Component) |
| 11.3 | Feishu (Lark) ✅ | Multi-Channel Access | Feishu bot SDK integration. Status: 首个 ChannelBase 真实实现——暴露当前 SPI 类型擦除（`raw: object`）等问题并推动 SPI 演进；webhook 签名验证 + tenant_access_token + 互动卡片。Wave 2 中国 IM（11.4/11.5）复用此模式，工作量 S each. | OpenClaw (Feishu channel), Hermes Agent (multi-platform gateway pattern), AgentScope (channels/feishu) |
| 11.9 | Slack ✅ | Multi-Channel Access | International channel. Status: Slack（M，ChannelBase 第二个实现，验证 SPI 通用性）。**Discord/Telegram（各 S）移入 P5**（2026-08-22 release 重分类；按需触发：trigger = 第一个明确要 Discord/Telegram 的国际客户需求，复用 Slack 模式）。 | OpenClaw (20+ channels), Hermes Agent (multi-platform gateway pattern) |
...
| 8.20 | Execution Replay & Debug Dashboard ✅ | Observability & Operations | Given a session, timeline-replay execution trace-partitioned by `Event.trace_id`: each superstep's channel changes (with values), tool calls (with arguments), LLM request/response (with messages), guardrail blocks (derived), SUBGRAPH_START/END (subgraph link). Vocabulary: `session` → `trace` → `event` (no "runId"). Built on EventStore (enriched by 1.3.19) + OTel TraceModel JOIN; web UI with DAG step-through + time-travel (fold-to-version + `derive_messages`); tenant-scoped; empty-log sessions hide the replay tab; "path A/C calls not in log" banner. **Phase 1** = execution replay (P3). **Phase 2** = version binding (data + code + eval version per trace, Palantir standard) — deferred to P5. **Depends on**: 1.3.19 ✅. **Seam notes**: 纯消费方、零 schema 改动——消费冻结请求 / 值级 delta / `STEP_END` / SUBGRAPH 引用对 / `trace_id` 与 OTel JOIN；worker 事件（LLM_REQUEST/RESPONSE、TOOL_CALL/RESULT）补齐 `trace_id` 透传；新增 `Execution identity per invoke` 保证 noop tracer 下分片仍有效；**回放覆盖范围 = Pregel 路径**（path A/C 跳过 Pregel，UI 横幅标注）。 | OMA offline Run Viewer, Conductor web dashboard, Salesforce Session Trace OTel API (session→turns→messages→LLM calls→actions), LangFuse Session/Trace/Observation, dsh `Session.deriveMessages()` JSONL log |
...
| 1.3.18 | Dynamic Orchestration ✅ *(see [ADR-032](../design/adr/032-dynamic-orchestration.md))* | Agent Runtime | 7th multi-agent pattern: coordinator node transforms goal + agent roster into a runtime task DAG, dispatches workers, synthesizes results. **范式定位**: data-as-plan —— LLM 产出**建议性** TaskDAG（Magentic "hint, not contract"），executor 确定性物化子图跑在 Pregel 上；业界裂解为 data-as-plan（OMA/Magentic/AgentScope/企业平台 coordinator）与 code-as-plan（Claude Code/Deep Agents interpreters/Hermes）两范式，Hecate 选前者（企业可审查/可回放/可治理诉求 + Pregel 免费提供确定性循环/分支，无需解释器）。**Phase 1**: `COORDINATOR` NodeType + `CollaborationPattern.DYNAMIC`（`infer_pattern` 识别）；TaskDAG 契约（goal/tasks/depends_on/expected_output/synthesis；fail-closed 前置校验：DAG 合法性 + 指派资格 `validate_task_requirements` + 能力不满足即拒不硬派）；executor 模板物化子图（独立 child session + SUBGRAPH_START/END，独立可回放）；Magentic 双循环图化（外循环 replan + 内循环分发 + evaluator 五问：完成？循环？进展？下一位？指令？；stall ≤2 触发反思；重规划 = append-only 计划修订（failed+recoveredByRevision / skipped+supersededByRevision 语义）+ coordinator prompt 每轮全新构建；replan-with-carryover：任务输出按 task_id 可寻址，新 DAG 可引用既有输出）；三轴预算（max_iterations=5 / stall_limit=2 / max_total_tasks=6 / max_concurrent=3 / orchestration 级 token 预算，任务边界检查，允许超一个 turn）+ additive `stop_reason`（token_capped/turn_capped/loop_capped/stall_capped，状态枚举稳定）+ capped 部分结果对 coordinator 可见指导（"reuse it, retry tighter, or raise budget"）；benefit-based 委派 rubric 进 coordinator prompt（仅在并行延迟/专家能力/上下文隔离收益明显超过启动/重复发现/综合/状态冲突/副作用成本时派发；**硬否决**：并行波次内输出依赖、重叠可变状态）+ prompt 快照测试钉死；五重隔离断言（子 session checkpoint 隔离、子任务记忆不回流父线程、仅显式声明输入映射、仅声明输出回写、错误判定只认状态契约不解析输出文本）；可选 per-task verifier（补 decision-matrix D6 空白）+ `ORCHESTRATOR_DECISION`/`ORCHESTRATOR_EVALUATION` 事件（typed blocker payload，additive，LogPolicy 默认入日志）；synthesis 节点支持声明式确定性 transform（复用 VARIABLE_SET 表达式，再进 LLM）；planner_model/evaluator_model/worker 模型分离（evaluator 默认小模型，对齐 DeerFlow non-thinking evaluator / OMA modelRouting）。**Phase 2（1.3.18a，P4）**: consensus proposer→judge 完整形态；append-only PlanPatch repair API（outcome-barrier + onPlanPatch 审批 + forward-only，OMA 语义）；异步编排 + 中途 steering（update/cancel；AgentScope inbox+wakeup 为参照）；plan 冻结 artifact + 精确重放（createPlanArtifact 等价物，与 8.20 配对）。**UI companion（P3，Phase 1 落地后的 follow-up change，无新 feature ID）**: pattern-selector 第 7 模式、canvas COORDINATOR 渲染与配置面板、8.20 回放 coordinator 卡片展开（goal+roster+DAG 修订史+逐任务轨迹）——挂 pattern-selector-ui / multi-agent-canvas / 8.20 增强注记。**Depends on**: 2.7a ✅ + Pregel ✅ + 1.3.19 ✅（子 session + log-as-truth substrate，plan 入日志零新持久化）；1.3.4（fail-closed 审批，Sprint 7 并行）。 | Magentic-One（arXiv 2411.04468，two-ledger 双循环源协议），OMA v1.14（goal-first 1:1 对标：PlanPatch/consensus/governance/hybrid routing），DeerFlow subagents/AGENTS.md（benefit-based routing + 三轴 cap + additive stop_reason，source-verified），Deep Agents interpreters（六种分发模式 + code-as-plan 动机），AgentScope（team tools + plan tools），dsh（Ralph 循环 / WorkerRun 台账 / SubagentRuntime 能力校验） |
| 1.3.19 | Event-Sourced Execution State / Log-as-Truth ✅ | Agent Runtime | (see [ADR-030](../design/adr/030-event-sourced-execution-state.md)). EventStore upgraded from observation log to state carrier: WAL 序（`append_batch` 先于 channel apply）+ `STEP_END`/`INTERRUPT` 提交点（撕裂尾部回退）；fold 经 `ChannelBehavior.write` 裁决（裁决后事实，`CHANNEL_WRITE_REJECTED` 为审计伴生）；`LogPolicy` 黑名单（`_route` 豁免）+ `LogInvariants` 运行时不变式注册表（STEP.BOUNDARY / DISPATCH.TREE，fail-stop）；checkpoint 降级为物化缓存（`channel_state + log_version`，节奏 = turn 结束 / interrupt / 每 N superstep）；resume 日志推导校验（懒建 Session 行）；会话级 TTL 清扫（30d/7d，interrupted 豁免，级联删除）；`PostgresCheckpointStore` 软废弃。**远期方向注记**：日志截断/压缩（EventStoreDB TruncateBefore / Kafka compaction，物化缓存 `log_version` 锚点已预留语义）；archive 归档层（MinIO/S3 冷存，策略枚举已留位）。**Depends on**: 8.8 EventStore ✅, CheckpointStore ✅. | dsh session log (source-verified v0.1.0-rc.5), deer-flow DeltaChannel (v2.0.0), MAF Durable extension, OMA checkpoint+resume |
...
| 7.2-abc | EvaluatorBase ✅ | Platform SPI | Evaluator uniform interface: `evaluate(input, context) → EvalResult`. Existing 40+ evaluators (7.2a) become `BuiltinEvaluator` implementations; third-party evaluators register via Plugin SPI. Elevates evaluation from hardcoded services to pluggable architecture. | Ragas (evaluator abstraction), Google ADK (evaluation metrics) |
| 11.1-abc | ChannelBase ✅ | Platform SPI | Channel adapter uniform interface: `receive`, `respond`, `stream`. REST/WS/CLI (11.1, 11.7) become `BuiltinChannel` implementations; Feishu/DingTalk/Slack etc. register as plugins rather than hardwired adapters. NotifierABC (8.6-abc) merged into ChannelBase — notification dispatchers become outbound Channel adapters registered via Plugin SPI. | OpenClaw (channel plugin architecture), Coze (channel SDK) |
| 10.3-abc | AuthProvider ✅ | Platform SPI | Auth provider uniform interface: `authenticate(token, db) → AuthContext | None`. JWT/APIKey (10.6) become `BuiltinAuthProvider` implementations; LDAP/OIDC/SAML register as plugins. SCIM directory sync (10.3b) extends this interface. | Enterprise standard (SAML/OIDC), Bisheng (SSO + LDAP) |
...
| 10.8 | Enterprise Vault Integration ✅ | Multi-Tenant & Enterprise | Integration with enterprise secret management platforms: HashiCorp Vault, AWS Secrets Manager, Azure Key Vault, Google Secret Manager. Per-agent identity authentication to vault via OAuth 2.0 token exchange. Dynamic short-lived credentials replace static API keys. SecretProvider abstracts backend differences. | HashiCorp Vault, AWS Secrets Manager, Azure Key Vault |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 5.5b ✅ | Plugin Packaging & Distribution | Tools & Plugins | Plugin packaging format (`.hecate-plugin` bundle), packaging CLI (`python -m hecate.plugin.cli package`), upload/install/uninstall UI, version management + upgrade workflow, installed plugin registry (beyond 5.5's local directory scan — handles packaged bundle extraction and registration). Marketplace distribution foundation (for P5 Asset Marketplace 12.0). **Depends on**: 5.5 (runtime engine) + TP5 (type ABCs for typed packaging). **Does NOT include**: marketplace UI (P5 12.0), plugin signing/security (P5 5.13). **Repositioned**: scope narrowed to P-tier deep-integration packaging (first-party/private plugins using the 8 type ABCs); no longer the ecosystem-facing third-party format — that role moves to 5.5c (Agent Plugins 1.0). Directory/git-URL install (ZIP demoted to transport-only) planned with 5.5d. | Claude Code Plugins (directory-based), OpenClaw (ClawHub marketplace + npm-pack), Dify (plugin marketplace + remote debugging) |
...
| 13.10-E4 | Saga/Compensation Pattern | 13.10 Temporal Conflict Resolution | Multi-step workflow rollback when step N fails; compensation actions for steps 1..N-1 | M |
...
| 7.3 | Workflow Evaluation | Evaluation & Testing | End-to-end workflow testing + regression testing | M |
| 7.4 | Human Annotation | Evaluation & Testing | Human review of Agent output, labeling and scoring | M |
| 7.4a | Human Score Calibration | Evaluation & Testing | Human override of evaluator scores with reason annotation | S |
...
| 4.14 | Memory Importance Scoring | Memory System | Access frequency, time decay, user marking, semantic relevance scoring | M |
| 4.15 | Multi-Signal Fusion Retrieval | Memory System | Vector + time decay + importance + frequency weighted fusion ranking | M |
| 4.16 | LLM-Managed Memory | Memory System | Agent self-manages memory lifecycle via function calls | M |
| 4.17 | Memory Pressure Alert | Memory System | System message at token threshold triggers memory consolidation | S |
| 4.25 | Layered Memory System | Memory System | Short-term / long-term (environment) / semantic (Qdrant) three-layer + Memory Flush Middleware | M |
| 4.21 | Task Memory | Memory System | Task trajectory learning: success/failure patterns, execution strategies; KM6 Work Context Graph | M |
...
| 5.4a ✅ | MCP Gateway | Tools & Plugins | REST/OpenAPI → MCP tool conversion (import-time projection + spec-driven virtual execution); external MCP server federation (`<target>__<tool>` naming, live-proxied via connection management); unified auth/routing/logging gateway on `/mcp` — caller-scoped `tools/list` (workspace-scoped DB API keys) and policy-pipeline-enforced `tools/call`; server-side credential brokering with redaction; egress baseline (HTTPS enforced, non-loopback rejected). Feature-flagged (`GATEWAY_ENABLED`, default off; flag-off behavior byte-identical). First production consumer of the tool policy pipeline (and fixed its `evaluate_visibility` loop bug in the process). v1 scope notes: SSE/streaming/webhook sources and OAuth credential flows deferred (5.8 / trigger features); legacy env API keys authenticate at platform scope. Change: `openspec/changes/archive/2026-09-08-mcp-gateway/`; specs: `mcp-gateway` (new), `mcp-server` + `tool-registry` (modified) | AgentArts gateway product card, Bedrock AgentCore Gateway (target model + credential brokering), fastmcp 4 Middleware |
...
| 5.9-enh | Skill Provider Registry | Tools & Plugins | Provider registry (project/user/bundled/custom origins) + rank precedence + kebab-case grammar + invocation policy separation | M |
...
| 1.3.10 | Multi-Level Intent Recognition ⊕ superseded by 6.23 | Agent Runtime | Hierarchical intent recognition: atomic intent (single user query) → workflow intent (multi-turn complex task) → session intent (overall dialogue goal). **⊕ Duplicate with 6.23 (5-Level Intent Recognition); 6.23 is the superset (adds caching + controller self-evolution). Merged into 6.23 — see Sprint 8 Opening Queue controller family (2.6a + 1.1.21 + 1.3.10⊕6.23).** | AgentArts (multi-agent controller), intent recognition |
| 1.3.20 | Agent Versioning & Channel Publishing | Agent Runtime | Agent-level versioning + channel binding, analogous to workflow versioning (1.1.9 ✅). Each agent has a version list with snapshot commit action; a channel (API / Web embed / Feishu / Slack / Webhook) is bound to a specific version snapshot, so runtime calls always invoke the locked version (immutable contract from end-user perspective). Status badge in editor (未提交 / 开发中 / 已发布 v3) + 「提交版本」 button + 渠道管理 tab parallel to workflow versioning flow. **Resource Versioning carrier (14.x mechanism)** — implements the versioning substrate that 5.9d (Skill Versioning) and 3.5.12 (Ontology Versioning) reference, and that 7.5 (A/B Testing) assumes. Reuses 1.1.9 storage + version-id format + diff/rollback machinery. **Source of the gap**: docs/research/2026-09-agentarts-product-comparison.md §3.3 — AgentArts makes 「提交版本 → 多渠道分享 (API/网页)」 a standard lifecycle step shared by all three agent forms (single/workflow/multi); catalog had no agent-level versioning entry before this row. **Depends on**: 1.1.9 ✅, channels system ✅ (channel/api/v1/, channel/slack/, channel/feishu/, channel/embed/), 1.3.15 Agent Environment ✅ (version snapshot scope). | AgentArts (提交版本并分享 lifecycle), Salesforce (Versioned Apex, version-bound deployment), LangGraph (thread versioning per deployment), OMA v1.14 (artifact versioning) |
...
| 2.13 | ACP (Agent Client Protocol) Support (NEW) | Multi-Agent Orchestration | External coding agents (Claude Code, Codex, Gemini CLI) as worker nodes in Hecate orchestration. Subagent provider seam (in-process / fork / ACP) with multiple coexisting providers. Complements A2A (agent-to-agent) — ACP is host-to-coding-agent. **Depends on**: 2.10 A2A ✅. | OMA (ACP support), Trinity (multi-runtime: Claude Code/Codex/Gemini CLI per agent), deer-flow (Claude Code ACP adapter), dsh (subagent seam, source-verified) |
...
| 5.9d | Skill Versioning | Tools & Plugins | Independent version management for skills: version snapshots, rollback to previous versions, version diff comparison. Aligns with Resource Versioning (14.x) mechanism (see 1.3.20 carrier). Enables safe skill updates without breaking dependent agents. | Claude Code Skills (version tracking), OpenClaw (ClawHub versioning) |
...
| 6.47 | Model Service Publishing | Model Management | Wire the published/unpublished lifecycle (already engineered as 6.45 Model Lifecycle Manager ✅ staging channels + promotion gates) into the provider/model management surface: `model_registry` gains a publish state (draft → testing → published); unpublished models are hidden from the application-facing reference surface (`GET /v1/models` → Create Agent Model dropdown, agent `llm_config` binding) while remaining testable via the inline Test Dialog; publish is an explicit action after a successful connectivity test / model test. Settings/models UI: status badge (已发布/未发布) + publish button + publish-state filter. Reuses 6.45 machinery — no new engine. **Gap source**: AgentArts makes 调测通过后发布 a standard lifecycle step (model service card shows 已发布 badge; unpublished services not referenceable by apps) — see docs/research/2026-09-agentarts-product-comparison.md 开发配置 row. **Depends on**: 6.45 ✅, model provider/registry tables ✅. | AgentArts (model service publish lifecycle), 6.45 Model Lifecycle Manager ✅ |
| 6.48 | Model Management Quick Wins | Model Management | Two low-cost items pulled forward from the AgentArts comparison (pure frontend + one aggregation query, no schema change): (1) list-level search/filter for providers and models on settings/models (AgentArts has keyword search on both levels; Hecate has none); (2) provider card call-count wiring — aggregate invocation counts from existing traces/costs data into the provider table (AgentArts provider card shows 调用次数; the data already exists in Hecate's traces, just not surfaced). Intended to ride along with any Sprint 8 change. **Gap source**: docs/research/2026-09-agentarts-product-comparison.md 开发配置 row. **Depends on**: traces/costs data ✅. | AgentArts (provider card stats, list search) |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 10.3b | SCIM Directory Sync ✅ | Multi-Tenant & Enterprise | User/group auto-provisioning via SCIM 2.0 protocol. Syncs from Azure AD, Okta, OneLogin to Hecate org/workspace membership. Extends AuthProvider (10.3-abc) with `sync_identity` capability — delegated CRUD on identity provider. | SCIM RFC 7643/7644, Azure AD SCIM, Okta SCIM |
...
| # | Feature | Domain | Description | References |
|---|---------|--------|-------------|------------|
| 6.23 | 5-Level Intent Recognition | Agent Intelligence | Merges 1.3.10 (multi-level intent recognition). Atomic intent → workflow intent → session intent layered recognition, with intent dynamic caching and controller self-evolution for performance and effect balance | AgentArts AgentStudio (formerly Versatile) |
| 6.49 | Intent Package Asset | Agent Intelligence | Intent packages as manageable data assets, paired with 6.23's recognition engine: each package = named set of intent categories + sample utterances per category (bulk entry CSV/JSON, import/export), referenced by the workflow advanced intent recognition node and controller intent routing (2.6a/6.23) as few-shot classification evidence to lift accuracy. Reuses evaluation-dataset storage patterns (7.2 family). **Gap source**: AgentArts 意图管理 — 意图包（意图分类+样例，批量录入，导入/导出）供工作流高级意图识别节点引用；Hecate 6.23 covers the runtime engine but had no sample-asset layer. Ships with the Opening Queue controller family (2.6a+1.1.21+1.3.10⊕6.23) or immediately after. **Depends on**: 6.23, 2.6a. | AgentArts (意图包/意图管理), 7.2 evaluation dataset machinery ✅ |
...
| 5.5d | Dual-Format Plugin Convergence & Export | Tools & Plugins | Single-package dual-format via the Agent Plugins `extensions` escape hatch: Hecate-private content (type/entry/permissions/config_schema + source) migrates into an `io.hecate/` reverse-domain namespace directory inside an Agent Plugins package (spec assigns no semantics to namespace contents; other clients ignore them) — one package is simultaneously a conformant Agent Plugins plugin (skills/mcp.json visible to all clients) and a Hecate deep-integration plugin. Eliminates dual-format friction without inventing a third format. Also `hecate plugin export`: package workspace skills as a conformant Agent Plugins bundle for use in Cursor/Codex etc. (bidirectional ecosystem play). Includes dropping forced-ZIP install in favor of directory/git URL (ZIP as transport only). Namespace domain must be one Hecate controls — confirm before implementation (decided 2026-08-16; io.hecate is a placeholder). **Depends on**: 5.5c (P3) + 5.5b ✅. Non-blocking enhancement. | Agent Plugins 1.0 spec §Client Extensions, npm/OCI "interchange envelope" pattern, Claude Code marketplace (git-repo format) |
...
| 1.3.21 | Engine Parity: Dynamic Fan-out, Time-Travel Resume, Declarative Interrupts (NEW — 2026-09-07 deer-flow/LangGraph engine comparison) | Agent Runtime | Close the three engine-level gaps found by diffing Hecate's Pregel against LangGraph (deer-flow's inherited engine; source-verified). Context: deer-flow itself uses almost none of these — its v1 research loop is sequential ``Command(goto)`` cycles with zero ``Send()`` fan-out, and v2 retreats to a single-agent + middleware harness — so the gaps are **vs the engine it inherits**, not vs the application. **① Declarative interrupts (S, 已交付 ✅ — change `openspec/changes/archive/2026-09-08-declarative-interrupts/`)**: compile-time ``interrupt_before`` / ``interrupt_after`` node lists — pause before/after named nodes without worker-authored ``Command(interrupt)``; plus ``remaining_steps`` exposed in ``execution_context`` so workers can degrade gracefully before ``MaxSuperstepsError`` (deer-flow's recursion_fallback pattern). **② Time-travel resume + update_state (M)**: extend ``CheckpointStore.load`` with ``checkpoint_id`` and ``PregelRuntime.execute`` with ``resume_from`` to restore from **any** historical checkpoint — the hard part already exists (1.3.19 log-as-truth ``fold_session`` rebuilds state at any ``log_version``); expose a state-mutation entry point (update_state equivalent) for re-planning. Feeds 6.26 E5 what-if checkpoint branching; upgrades 8.20 replay from read-only visualization toward executable time-travel. **③ Send-style dynamic fan-out (M/L)**: runtime branch registration — a conditional edge returns N dispatch packets, each with its own state slice, merged via the existing sub-channel + ConflictResolver (13.10) machinery; today's FAN_OUT is a compile-time static branch table. Unlocks 5.11 Deep Research parallel collection and map-reduce aggregation. **Ride-along sub-items (S)**: node-level CachePolicy (TTL + key_func; extends 5.7 tool caching to CONVERSATION/KNOWLEDGE nodes — LangGraph ``add_node(cache_policy=…)``); pluggable ACCUMULATOR reducers beyond the built-in ``add``. **Already-planned neighbors, deliberately not duplicated here**: debug/tasks/checkpoints stream modes → 11.18; superstep inspector UI → G7 (1.1.23); cross-thread store → 4.23. **Explicitly not adopted** (research conclusion): Functional API (``@entrypoint``/``@task``) — Hecate's GraphDSL + template library covers the same expression; delta-checkpoint cache layer — 1.3.19 log-as-truth + materialized checkpoints is the stronger answer (log authoritative, cache discardable). **Depends on**: Pregel ✅, 1.3.19 ✅ (② folds the log), Command/interrupt ✅ (① extends), FAN_OUT/MERGE ✅ + 13.10 ConflictResolver ✅ (③ generalizes). **Execution order: ① → ② → ③** (smallest first; ② reuses existing fold machinery; ③ is the largest and benefits from ①'s test breakpoints and ②'s checkpoint anchoring). **① 交付内容** (2026-09-08, 22 new tests, full suite 4317 green): DSL 顶层 ``interrupt_before``/``interrupt_after`` 数组(schema + parse + ``to_json`` roundtrip);编译期校验(node 存在性、task mode 拒绝);引擎 superstep 边界**整步**暂停(before=dispatch 前、after=WAL append+apply 后);phase-aware resume——中断描述符(``kind``/``phase``/``nodes``/``interrupt_id``/``superstep``/``remaining_steps``)由日志最后一个未闭合 ``INTERRUPT`` 事件推导(SHALL NOT 依赖缓存 metadata),before → 执行被暂停节点本身(一次性豁免防断点自咬),after → 沿中断步全部执行节点出边并集(``_route`` 从恢复通道读取);``remaining_steps`` 进 ``execution_context``;worker-authored 中断 WAL 提交序修复(以 ``INTERRUPT`` 为提交点,同步骤兄节点写入不再丢弃,缓存不再领先日志);session status 事件接线(``INTERRUPT`` → ``interrupted``,resume 端点双门通过)——**HITL 链路第一次端到端可达,零自定义 worker**。行业定位(2026-09-08 调研,19 项目):图级声明式 HITL 仅 LangGraph 静态 breakpoints、Dify ``HumanInputNode``、IBM watsonx User Activity、Google ADK ``RequestInput`` 有先例;从事件日志 fold 推导 resume 为 Hecate 独有。**② design 定稿**(change `openspec/changes/time-travel-resume/`,待实施):锚点为**日志提交点**(`STEP_END`/`INTERRUPT`/新增 `FORK` 的版本号,非 checkpoint_id——生产 CheckpointStore 无历史且 ADR-030 已降级缓存,本行上文"extend ``CheckpointStore.load``"措辞以此为准修正);fork = 新子会话 + 单条 `FORK` 快照事件引导(子日志自包含 + lineage,父会话零侵入);update_state = 追加记录式 `CHANNEL_WRITE`(`source="update_state"` + actor,`STEP_END` 收尾防撕裂尾部丢弃)。**② 延后跟进项**(design Open Questions,防遗忘):FORK 载荷压缩(eviction 感知裁剪 vs blobs 拆表,待真实载荷分布;EventStore 无载荷大小约束,仅 JSONB 物理上限);悬挂 TURN 清扫 × update_state 门控联动(需 liveness/崩溃检测;硬杀盲区为既有状况,非 ② 引入);commit-points 列表富化(消息预览/token 消耗,8.20 消费侧);fork 配额/同父分支上限(平台配额层统一;fork 执行复用既有预算门,无绕过敞口)。**② 交付内容** (2026-09-09,46 new tests,full suite 4360 green,change `openspec/changes/archive/2026-09-09-time-travel-resume/`):锚点为日志提交点(``STEP_END``/``INTERRUPT``/新增 ``FORK`` 的版本号,非 checkpoint_id——生产 CheckpointStore 无历史且 ADR-030 已降级缓存,本行上文"extend ``CheckpointStore.load``"措辞以此为准修正);``GET /sessions/{id}/commit-points``(纯日志派生,缓存无关);fork-and-run——``POST /sessions/{id}/fork`` 创建子会话,日志以单条自包含 ``FORK`` 快照事件引导(lineage + 折叠态 + 推导续跑集),父日志零侵入,``GET /sessions?parent_session_id=`` 列举同源分支;update_state——``POST /sessions/{id}/state`` 追加记录式修改(``CHANNEL_WRITE`` 带 ``source``/``actor`` + ``STEP_END`` 收尾,执行中 409 含 ERROR 逃逸,reducer 语义);引擎 ``resume_from`` tail-only 守卫(历史点必须 fork);副作用重执行如实声明(不回滚不补偿)。**实现期修复(D11)**:initial_input 写入从未入日志——fold 的 marker 检查原作用于全部事件而簿记事件无 marker,投影等价校验被 NonReplayablePrefix 早退掩盖而空转;修复后 marker 检查收窄到状态携带事件,initial_input 经 WAL 以 ``STEP_END(source="initial_input")`` 收尾,① 的等价测试首次实跑通过。延后跟进项见上文。**③ 交付内容** (2026-09-09, 12 new tests, full suite 1090 green, change `openspec/changes/archive/2026-09-09-dynamic-fanout/`):planner 节点 CONDITION + `fanout` DSL 块(`over` 通道逐元素生成 `_dispatch` 包,`target`/`state_key`/`max_fanout` 编译期校验),引擎调度单元升级为 `Invocation`(同同子节点同 superstep 并行 N 次,语义去重被刻意保留);`_dispatch` 受日志控制(`should_log_channel` 显式豁免,同 `_route`),`STEP_END` 的 `fanout` 段携带所有分支子通道快照,`fold_session` 重建——**T2b 不变式**("扇出分支输出必须活过复续跑/fork")对静态 FAN_OUT 与动态扇出同步闭合(`FORK` 载荷保真,缓存缺失 log-only 恢复后 MERGE 正确);`continuation.py` 的 `_dispatch` 采纳规则带 live-planner 守卫(planner 节点属于最近 STEP_END 锚点的 executed 列表才采纳计划,陈旧计划不被采纳);`fanout.max_fanout`(节点级,默认 64,上限 1024)+ superstep 总量(256)分层上限,fail-closed `FanoutLimitError`;per-branch 容错粒度(`on_branch_error: fail_fast | collect`,collect 模式失败分支记录 `__branch_error__` 条目,整批继续);`_execute_merge` 动态源路径扫描 `_fanout__{source}__idx{i}` 聚合,静态路径 `{branch_id: result}` 语义逐字不变;可插拔 ACCUMULATOR reducer(模块级 `_REDUCERS` 注册表 + 内建 `add`/`append` + 编译期未知名报错,移除未知 reduce_fn 静默 overwrite 回退);①+②+③ 闭环——planner ∈ `interrupt_after` 实现 HITL 计划评审(描述符 nodes=[planner],恢复后按计划派发);target ∈ `interrupt_before` 对整批调用一次预中断(描述符 nodes=[target_node])。行业定位(2026-09 调研,17 项目三层覆盖):LangGraph `Send` 为 canonical(并承其 #6789 序列化坑——`Send` 对象与 checkpointer 二选一),本项以受日志控制 channel 天然规避该坑——可序列化、入 WAL、可重放三者兼得;ADK dynamic workflows `node(worker,{parallelWorker:true,maxParallelWorkers:4})`、Palantir AIP Loop、OpenJiuwen `BranchComponent.add_branch(condition, target=list)`、Dify `is_parallel` 为数据驱动 N 主流;AgentScope `AgentCreate` 工具、Claude Code `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS=20`、OpenClaw 多 lane 上限、DeepSeek Harness `agentTeams`、`Temporal` `Promise.all` over `ExecuteChildWorkflow` 提供跨进程/分布式先例;reducer 注册机制与各家合并策略同源(LangGraph `Annotated[list, op.add]`、Dify `output.items`、Palantir Loop list、Temporal `Promise.all`),ACC的可可插拔 reducer 是 map-reduce 真实前置件(ride-along),**显式排除** LLM 规划器产出 N(方案 B,留 5.11 Deep Research)与 `Command(goto=[Send...])`(方案 C,后置)。与 1.3.18 COORDINATOR 划线:后者为跨 superstep、隔离子会话、自带 LLM 规划器的重型编排,③ 限单 superstep 图内并行,两者是不同层次动态性;Pydantic 风格的 FastAPI 路径不变——main path 与 design 已锁定为同 superstep 单调度面。**③ 延后跟进项**(防遗忘,挂到 design Open Questions):平台配额层与 `FanoutLimitError` 联动口径(同 ② fork 配额同款,留平台配额统一);大切片 offloader 集成(v1 仅文档声明 + 上限封顶,待真实载荷分布);`fanout` 配置 v1 限 CONDITION,FAN_OUT 节点动态模式 (`mode: dynamic`) 是否开放待 5.11 接线时按需评估。 | LangGraph Send / Command / interrupt / get_state_history / update_state / CachePolicy (docs.langchain.com, source-verified), deer-flow v1 sequential research loop + v2 lead-agent retreat (source-verified), Hecate engine audit 2026-09-07 |
