...
| `ask` | The tool pauses for human approval via `interrupt()` (see [Human-in-the-Loop tutorial](../tutorials/06-human-in-the-loop.md)) |
...
- [Human-in-the-Loop tutorial](../tutorials/06-human-in-the-loop.md) — what happens when a tool is matched by an `ask` rule
