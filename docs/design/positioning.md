...
| **Runtime hallucination mitigation** | ✅ citation provenance layer: tool-result chunk markers + session-resolvable registry + uncited-claim risk signal (1.3.5e Stage 1); entailment scoring staged next | ❌ | ❌ | ❌ | Partial (Einstein Trust Layer grounding, no public scoring API) | ✅ Guardrails contextual grounding check (caller supplies the grounding source) | ❌ |
