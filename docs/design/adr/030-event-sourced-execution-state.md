...
| **4.13 durable compaction** (spec'd by ADR-033, implementation sequenced) | `EventType` enum (additive, unknown reads fall back to `CUSTOM`) + derive-messages path | Add the four `COMPACTION_*` / `CONTEXT_SURFACE_REPLACED` event types; `CONTEXT_SURFACE_REPLACED` participates in the derive-messages fold only (channel fold unchanged) | Bracket lock enforced (`START` without `COMPLETED` = in-progress); originals are never deleted; `COMPACTION_COMPLETED` is a checkpoint source |
