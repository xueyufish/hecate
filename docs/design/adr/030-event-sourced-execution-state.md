...
| **4.13 durable compaction** (spec'd by ADR-033, implemented 2026-09-19) | `EventType` enum (additive, unknown reads fall back to `CUSTOM`) + derive-messages path | Add the four `COMPACTION_*` / `CONTEXT_SURFACE_REPLACED` event types; `CONTEXT_SURFACE_REPLACED` is a bookkeeping ledger entry — the substitution applies in the per-invocation derived view (the messages channel keeps every original; channel fold unchanged) | Bracket lock enforced (`START` without `COMPLETED` = in-progress); originals are never deleted; `COMPACTION_COMPLETED` is covered by the next superstep checkpoint (metadata records `last_compaction_id`) |
