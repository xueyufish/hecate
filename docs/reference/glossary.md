...
- **Extension Point** — an abstract interface (ABC) in the engine layer that lets you customize execution. Hecate defines 26 engine extension interfaces plus 6 optional SPI methods on `EnginePort`, and 8 plugin SPI types at the platform layer. See [Extension Points Reference](extension-points.md).
