...
- **Superstep** — one iteration of the Pregel loop: ready nodes read channels, the worker pool dispatches them, the runtime awaits all results, applies writes, persists a checkpoint, and evaluates conditional edges. Execution ends when no more nodes are ready.
...
- **Node** — a typed execution unit in a graph. Hecate defines nine node types: `conversation`, `knowledge-retrieval`, `tool-call`, `agent`, `condition`, `variable-set`, `fan-out`, `merge`, `suggestion`. See [Graph DSL Reference](graph-dsl.md).
...
- **Checkpoint** — an immutable snapshot of all channel values, the current node, the superstep number, and execution metadata, stored in PostgreSQL (with the latest cached in memory). Enables time-travel debugging and resumable interruptions. See [Extension Points](extension-points.md#4-checkpointstore).
...
- **Extension Point** — an abstract interface (ABC) in the engine layer that lets you customize execution. Hecate defines 11 core extension points plus 4 optional SPI methods on `EnginePort`. See [Extension Points Reference](extension-points.md).
- **SPI (Service Provider Interface)** — the 4 optional methods on `EnginePort` (`context_assemble`, `evidence_query`, `agent_execute`, `tool_execute_sandbox`) that ship with default implementations and can be overridden by service-layer adapters.
- **`interrupt()`** — pauses execution at a superstep barrier to wait for external input (typically human approval). The runtime writes a checkpoint and stops; resources are released while paused.
- **`Command`** — resumes an interrupted session by injecting the user's input and continuing the superstep loop from the checkpoint where it stopped.
- **Session** — one conversation lifecycle: `active` → `interrupted` → `active` → `completed` (or `failed`). An interrupted session holds no resources — it is a checkpoint waiting to be resumed.
...
- **A2A (Agent-to-Agent Protocol)** — the Linux Foundation v1.0 GA standard for cross-framework agent communication. Hecate agents are A2A-discoverable via Agent Cards and invokable via the A2A task lifecycle. See [Enable A2A Server](../how-to/enable-a2a-server.md).
