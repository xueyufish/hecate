...
- **Default merge strategy is rebase** — when merging a feature branch into `main` (PR button or local `git merge`), use **rebase merge** (`--rebase`) or **squash merge** by default; avoid **merge commits** (`--no-ff` / GitHub's "Create a merge commit") because the resulting topology accumulates redundant merge bubbles that obscure `git log --graph` and make bisect / blame noisy. Exception: a long-lived integration branch (multi-week merges) where the merge commit's audit trail has independent value may use a regular merge commit; this should be rare. Rationale: rebase keeps `main` linear, which is the project default and matches plan §1.1's GitHub Flow framing.
