...
from hecate_memory.rag.vector_store import VectorStore, _rrf_fuse
...
class TestGetVectorStoreFactory:
    def test_returns_qdrant_store_by_default(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("hecate_memory.rag.factory.settings.VECTOR_STORE_TYPE", "qdrant")
    ...
    def test_returns_chroma_store(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("hecate_memory.rag.factory.settings.VECTOR_STORE_TYPE", "chroma")
        from hecate_memory.rag.chroma_store import ChromaVectorStore
        from hecate_memory.rag.factory import get_vector_store
