...
from hecate_memory.rag.crawler import CrawlResult, WebCrawler
...
async def test_ingest_url_endpoint(client: AsyncClient) -> None:
    ...
    with (
        patch("hecate_memory.api.knowledge.web_crawler.crawl_urls", return_value=[mock_crawl]),
        patch(
            "hecate_memory.api.knowledge.knowledge_base_service.ingest_document_text",
...
async def test_ingest_url_batch_endpoint(client: AsyncClient) -> None:
    ...
    with (
        patch("hecate_memory.api.knowledge.web_crawler.crawl_urls", return_value=mock_results),
        patch(
            "hecate_memory.api.knowledge.knowledge_base_service.ingest_document_text",
