...
SecurityAuditService uses async_session_factory directly). They are
...
class TestSecurityAuditAPI:
    async def test_query_endpoint_returns_empty(self, client: AsyncClient):
        response = await client.get("/api/security/audit?agent_id=nonexistent")
    ...
    async def test_query_endpoint_pagination_params(self, client: AsyncClient):
        response = await client.get("/api/security/audit?limit=10&offset=0")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_query_endpoint_rejects_invalid_limit(self, client: AsyncClient):
        response = await client.get("/api/security/audit?limit=0")
        assert response.status_code == 422

    async def test_query_endpoint_rejects_negative_offset(self, client: AsyncClient):
        response = await client.get("/api/security/audit?offset=-1")
        assert response.status_code == 422
